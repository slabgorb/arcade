...
- **⚠ Use SWEPT (path) collision for any bullet↔target pair, not endpoint-only.**
  A player shot travels **111+ lo-units/frame** (`bullet.ts` `BULLET_SPEED` + ship
  velocity) and the saucer/ship hitboxes are small — the same geometry that let
  fast shots **tunnel straight through small rocks** (shipped bug, fixed
  2026-07-04). `sim.ts` already exposes **`sweptOverlaps`** (segment-vs-AABB in the
  target's local toroidal frame, degrading to the point test when the bullet is
  motionless) — the bullet-vs-rock loop now uses it. **Reuse it verbatim** for
  player-bullet↔saucer AND saucer-bullet↔ship (saucer shots also fly at
  `SAUCER_BULLET_SPEED` 111/frame). An endpoint-only check WILL tunnel these pairs
  exactly as it did small rocks. Direct saucer↔ship and saucer↔rock contact move
  slowly enough that plain `overlaps` is fine.
...
- **No tunneling (regression guard for the small-rock bug):** bullet↔saucer
  and saucer-bullet↔ship are tested with **MOVING** bullets — a shot at cardinal
  muzzle speed (111 lo-units/frame) whose path crosses the target *this frame*
  registers the hit even when neither the pre- nor post-move endpoint sits inside
  the hitbox. Every existing A-8 collision test fires a **stationary** bullet
  already overlapping the target, which is precisely why the small-rock tunneling
  bug shipped unseen — do not repeat that fixture pattern for the saucer pairs.
...
_Amended by Dev (Julia) 2026-07-04 after a playtest bug fix: player-bullet-vs-rock now uses swept (path) collision (`sim.ts` `sweptOverlaps`) because fast shots (111+ lo-units/frame) tunneled through small rocks (84-unit window) under the old endpoint-only test. A-13's bullet↔saucer and saucer-bullet↔ship pairs share that geometry — reuse `sweptOverlaps` and test with MOVING bullets (see the added Code-shape note and acceptance criterion)._
