...
## Carry-forward from A-6 / A-7 (both done 2026-07-03)

Now that A-6 (rock entities) and A-7 (splitting) are merged, two of this doc's forward
assumptions can be pinned to real code (source: `sprint/archive/A-6-session.md`,
`sprint/archive/A-7-session.md`):

- **Splitting stays out of A-10.** `splitRock` lives in `core/rocks.ts`; A-10 spawns
  `size: 'large'` only and never calls it — confirmed, no scope change.
- **RNG-mutation discipline (now verified in code):** A-6's `spawnRock` **and** A-7's
  `splitRock` both **mutate the passed `rng` in place**. `updateWaveDirector`/`spawnWave`
  run inside `stepGame`, so they must **clone `state.rng`** before spawning (the `sim.ts:24`
  pattern) — never advance `state.rng` directly. This is the concrete mechanism behind this
  story's "all randomness through `state.rng`" AC.
- **⚠ A-6's actual `spawnRock` signature differs from this doc's guess.** A-6 shipped
  `spawnRock(rng: Rng, size: RockSize, bounds: Bounds): Rock`, and it **picks a random
  position *inside* `bounds`** — it does **not** accept a caller-supplied edge
  position/heading. So the sketched `spawnRock(rng, 'large', position, heading)` composition
  with `pickEdgeSpawn` **will not work as written**. A-10 must either construct the edge
  `Rock` literal directly (position from `pickEdgeSpawn`; velocity from heading × a large-tier
  speed) or extend `spawnRock` with an optional placement argument — the count / trigger /
  timer logic in this story is unaffected either way. Re-check `spawnRock`'s shape in
  `core/rocks.ts` at implementation time.
- **Per-frame velocity units** (world-units per 60 Hz frame) are unchanged; nothing A-6/A-7
  added breaks this story's caps or determinism.

---
_Generated by `pf context create story A-10` from the sprint YAML._
_Enriched by Winston Smith (SM) 2026-07-03: appended the A-6/A-7 carry-forward — clone
`state.rng` before spawning (spawnRock/splitRock mutate it), and the ⚠ real `spawnRock(rng,
size, bounds)` signature (randomizes position) vs this doc's edge-placement guess. Source:
`sprint/archive/A-6-session.md`, `sprint/archive/A-7-session.md`._
