...
**The gap this epic closes.** df1–df6 built every PURE core but wired none of it into a
lifecycle: no attract mode, no start-of-game, the `men<0` game-over
(`endgame.ts isGameOver`, `defender/DEFA7.SRC:1423`) is computed but never acted on, the
df5-1 scanner projection is drawn **nowhere**, there is no HUD, and the hall-of-fame board
persists with no name-entry flow reaching it. df7 is phase 6-7, the **wiring** that makes
Defender a cabinet you play attract → game-over → hall-of-fame.

**The model: pure-first, wired-after (pm4/mc6).** The missile-command mc6 attract +
state-machine epic and the pac-man pm4 cabinet lifecycle are the direct siblings. The
phase machine is a **pure, seeded, clock-free** transition function in `src/core` first,
then wired to the shell. df5 Decision C ruled the end-of-game pure-first *here*; df5
Decision D deferred 2P alternating handoff *here*. Both land now.

**Build order (load-bearing).** The spine first, then the transitions that hang off it:
1. `df7-1` phase machine core (`phase.ts`) → 2. `df7-2` setup→play start → 3. `df7-3`
self-playing attract demo → 4. `df7-4` hall-of-fame + name entry → 5. `df7-5` HUD +
scanner render → 6. `df7-6` 2P handoff → 7. `df7-7` lobby showcase + full-lifecycle
playtest.

**Five rulings (full text in the epic description / `sprint/epic-df7.yaml`):**
- **A** — the phase machine is **pure-first** and **consumes** df5-6 `isGameOver` (men<0);
  it does not re-decide game-over.
- **B** — **ADR-0005 binds every transition**: no full-frame strobe in any attract/death/
  game-over presentation; the df4-2 render guard stays green across the whole lifecycle.
- **C** — the hall of fame is `@shared/highscore` + `@shared/name-entry`, **consumed** —
  no bespoke high-score UI, no new persistence.
- **D** — 2P alternating handoff lands here; the **cocktail screen-flip is documented-not-
  ported** (desktop-only, upright).
- **E** — lobby-showcase opt-in via the existing showcase seam — no new lobby infra.

**Cross-story guardrails every df7 story carries:**
- The phase machine / attract driver are **pure, seeded, clock-free** (purity.test.ts
  green); the shell drives them — no per-phase rAF.
- Every `src/core` constant is gated by a `claims/*.json` entry citing
  `defender/<FILE>.SRC:<line>`; line numbers from tool output only; RASM radix.
- **ADR-0005 everywhere** (Decision B) — no whole-framebuffer inversion in a single frame,
  in any phase; accessibility outranks ROM fidelity.
- The hall of fame and name entry are `@shared` **consumed, not re-built** (Decision C);
  no new `@shared` extraction anywhere in df7.
- The scanner is **df5-1's projection finally drawn** (df7-5) — not a re-derived minimap.
- Colour by `df2` palette **INDEX** only.

**Relationship to df5's open stories.** df5-7 (visual playtest) is still in df5 backlog and
shows the *gameplay loop*; df7-7 shows the *lifecycle* (attract/game-over/hall-of-fame
screens) — different surfaces, both kept. df7 consumes df5-6 (endgame/highscore), df5-1
(scanner), df5-3 (score/men) and df6 (event cues); those cores should be **done** before
df7's wiring is kicked off, so its exact HUD/transition seams are sharp.

**Design spec:** to be written at kickoff —
`docs/superpowers/specs/2026-08-XX-defender-df7-lifecycle-design.md` (mirror mc6/pm4 +
the df5 spec). Roadmap parent:
`docs/superpowers/specs/2026-08-13-defender-cabinet-roadmap-and-df1-design.md` sec 4 df7
(≈18 pts). df7 is the **final content epic**; df8+ is reviewer-driven hardening.
