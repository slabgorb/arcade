...
**Build order (load-bearing).** The two pure structural seams every later story reads
come first, then the economy and panic that hang off enemy deaths, then the powers, then
the end of the game, then the eyes:
1. `df5-1` `scanner.ts` (RED-first) → 2. `df5-2` `waves.ts` → 3. `df5-3` `score.ts` (men
   counter) → 4. `df5-4` rescue + panic → 5. `df5-5` `powers.ts` → 6. `df5-6` end-of-game
   core → 7. `df5-7` visual playtest.

**Four rulings (full rationale in the design spec §3–6):**
- **A** — the scanner ports `SCNR`'s world→radar projection, not a re-derived minimap (`df5-1`).
- **B** — smart-bomb/hyperspace **cite** ADR-0005; the `df4-2` render guard stays green (`df5-5`).
- **C** — end-of-game is pure-first here, phase-wired in `df7` (`df5-6`).
- **D** — 2P alternating handoff defers to `df7` (scope).

**Cross-story guardrails every df5 story carries:**
- Every `src/core` constant is gated by a `claims/*.json` entry citing
  ...
  bare decimal).
- Everything is a `df3` scheduler process (`NEWP,STYPE`) — no per-subsystem `rAF`/tick.
- Colour by `df2` palette **INDEX** only — no hex literal.
- Scoring identity is a cited dossier task (**values**, not names) — a wrong point value
  ships GREEN; anchor to per-value `expectPopulated` + mutation-prove.
- The scanner and collision read the **ONE** `df3` `world.ts` model — no second coordinate
  system.
- The hall of fame is `@shared/highscore` + `@shared/name-entry` — **consumed, not
  re-implemented**; no new `@shared` extraction anywhere in df5.
- ADR-0005 is **cited, not re-decided** — no effect writes a whole-framebuffer inversion in
  a single frame.

**Shared-surface note (the jt9 file-surface rule):** `df5-3` owns the men counter; `df5-6`
reads it for the `lives<0` game-over. Different files — commit coherently, do **not** merge
the two stories.
