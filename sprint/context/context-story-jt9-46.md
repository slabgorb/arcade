...
WHAT THE PORT DOES NOW. drawList (sim.ts:2284-2286) emits exactly ONE op per enemy — the
buzzard-body frame enemyFrame(p) returns (BRFLAP / BRRUN1-4 / BRSTND, all sourced from the
BRUN*/BFLY* BIRD sprites, pictures.ts:1685-1693). A PLAYER (sim.ts:2281-2283) instead loops
playerDrawList(p), which returns TWO stacked sprites: the bird mount PLUS the knight rider
(PLYR1 / PLYR2, sim.ts:2201-2210). So the player has a rider and the enemy has none — every
...
too. Keep it in the pure render-SELECTION seam (sim.ts:2111 "routing≠geometry"): it returns
...
OUT OF SCOPE, FILE SEPARATELY: remountEnemyProcess (sim.ts:1056-1082) HARDCODES type:'bounder',
...
## Acceptance Criteria
- AC-1 EVERY ENEMY DRAWS A MOUNT AND A RIDER. A pure enemyDrawList(p) returns TWO ordered sprite names for a kind:'enemy' process — the buzzard-body frame (unchanged from enemyFrame) and the species' DPLYR rider — and drawList emits both, tagged with the enemy's facing, mirroring the player path (sim.ts:2281-2286 vs the current single-op :2284-2286). Guard: a fresh wave's enemy produces a rider op stacked on its BR* mount. MUTATION: dropping the rider op (reverting to the single mount op) must redden a named test.
