...
**The gap this epic closes.** df1–df5 built a fully playable but **silent** Defender:
the world scrolls, the menagerie attacks in escalating waves, the scanner reads the
world, humanoids are abducted/rescued, the two emergency powers fire, and the game ends
into a hall of fame — with no audio at all. df6 is phase 5, the **audio**.

**Why it is the "absent-source" epic.** The sound board is a separate M6808
(894.886 kHz, `williams.cpp:1540`) running `defend.snd` = `video_sound_rom_1.ic12`
(`williams.cpp:2002`), and **no sound-board source is vendored** — the same gap Joust's
tree has. `ROMF8`/`ROMC0`/`ROMC8` are main-CPU control/diag ROMs, not sound source. So
df6 does **not** emulate the board; it follows the **battlezone/jt5 synthesis** route:
pin the command values the main CPU writes (cited), synthesise each effect informed by
MAME's board model + the machine-local `defend.snd` bytes, and route everything through
the fleet's `@shared/audio` seam.

**Ground truth is the main-CPU SOUND TABLE, and it is fully cited.** The game writes a
6-bit command to `SOUND EQU $CC02` (`defender/PHR6.SRC:18`; `*B0-B5 SOUND :133`;
`*B1 THRUST :138`) via `SNDOUT` (`defender/DEFA7.SRC:693-704` → `STB SOUND :703`). Every
cue is one `FCB` entry in the `*SOUND TABLE` (`defender/DEFA7.SRC:659-691`), documented
at `:660` as `SNDPRI,N*(REPCNT,SNDTMR,SND#)` (`SNDTMR` in 16 ms; `SNDPRI`
"HI=HI,INTERRUPTABLE BY EQUALS" `:662`). `SNDLD :706-721` + `SNDSEQ :723-757` (called
`:1943`) drive the priority/repeat model and the `THFLG` thrust held-state (on `$16 :750`,
off `$0F :743`). This is a **richer, better-documented command surface than jt5 had** — no
cue in df6's core need be an invention.

**Build order (load-bearing).** Seam-with-emitters first so the seam is never dead, then
the two stateful cues, then the samples, then the ears:
1. `df6-1` seam + single-shot emitters (`events.ts`/`audio.ts`/`audio-dispatch.ts`) →
2. `df6-2` the thrust held-loop + lander-suck repeat →
3. `df6-3` synthesise + upload the samples (live-200) →
4. `df6-4` audible playtest.

**Five rulings (see the epic description for full text):**
- **A** — df6 is SOUND ONLY; the two ATTRACT musics (PHANTOM `AMODE1.SRC:148`, TOCCATA
  `:152`) belong to attract mode and **defer to df7**, citations preserved.
- **B** — the ASSET story's acceptance is a **live 200, not a green vitest**
  (`@shared/audio` degrades silently on a 404 — the sw3-5→sw8-14 gap); findings are
  **filed** before the epic closes.
- **C** — cues are **DATA on the sim state, never callbacks**, so purity holds and df3's
  seeded-determinism replays reproduce bit-for-bit (a test pins it).
- **D** — **ADR-0005 is a VISUAL exception and does NOT mute audio**; smart-bomb/death/
  panic cues play normally.
- **E** — each cue's `SNDPRI` byte maps onto the `@shared/audio` **CHANNELS** voice-
  stealing seam; the 6809 `SNDSEQ` sequencer is **not** re-emulated.

**Cross-story guardrails every df6 story carries:**
- Every `src/core` sound constant is gated by a `claims/*.json` entry citing
  `defender/<FILE>.SRC:<line>`; line numbers from tool output only; RASM radix (`$hex` vs
  bare decimal — the SOUND TABLE `FCB`s are hex).
- Cues are DATA on the returned sim state (Decision C) — no callbacks, no new RNG draw,
  no ordering change; `purity.test.ts` stays green; the df3 determinism replay is pinned.
- `@shared/audio` is **consumed, not re-implemented** — the game supplies only its
  `SOUNDS`/`CHANNELS` NUMBERS (SH2/SH4: share the VERB, not the NUMBERS); no new `@shared`
  extraction anywhere in df6.
- Every declared cue has an emitter — a seam with no emitter is the **jt5-1 blind spot**
  (six cues were deletable with the suite fully green); **mutation-guard every emitter**.
- The dispatch has a `never` exhaustiveness guard (SH4-5,
  `tests/audio-dispatch-convention.test.mjs`) — a new kind without a cue is a compile error.
- The M6808 is **synthesised, not emulated** (no board source is vendored); no cue is
  silently fabricated as authentic — un-anchored cues are marked invention-pending.

**Shared-surface note (the jt9 file-surface rule):** `df6-1` and `df6-2` both touch
`events.ts`/`sim.ts`/`audio-dispatch.ts`/`audio.ts`, but `df6-2` adds a distinct STATE
mechanic (a loop + a sustained repeat with edge detection) the single-shot path cannot
express — a coherent second story, **not** a false split. `df6-3` (assets) and `df6-4`
(playtest) are disjoint surfaces.

**Design spec:** to be written at kickoff —
`docs/superpowers/specs/2026-08-1X-defender-df6-sound-design.md` (mirror the df5 spec).
Roadmap parent:
`docs/superpowers/specs/2026-08-13-defender-cabinet-roadmap-and-df1-design.md` sec 4 df6
(≈13 pts).
