...
- `[VERIFIED]` **SND-114 is right now.** `grep -rn '^52[$]:' reference/original-source/centipede` → 20 tree-wide; in revision.v4, `CENTI4.MAC:622`, `:1076`, `:2065` and `CENIR4.MAC:388` besides SOUNDS' own `:2418`. Four, as the claim now says, with all four locations named. Both specialists reproduced this.
