...
describe('sw6-1 AC-7 — the bake is headless and reproducible', () => {
  ...
  // Scrape one `export const X = { … } as const` literal out of audio.ts, bounded
  // to the literal itself.
  //
  // ⚠ The previous form took a COUNT and stopped after that many matches — which
  // is how finish_ground.wav hid in plain sight for two stories: the manifest grew,
  // the count did not, and the assertion simply stopped reading before it reached
  // the new entry. A count is a promise to remember; this bound is a fact about the
  // file. Line comments are stripped so prose can never be mistaken for an entry.
  const scrapeManifest = (audio, marker) => {
    const start = audio.indexOf(marker)
    if (start === -1) throw new Error(`manifest scrape: ${JSON.stringify(marker)} not found in audio.ts`)
    const end = audio.indexOf('\n}', start)
    if (end === -1) throw new Error(`manifest scrape: no closing brace for ${JSON.stringify(marker)}`)

    const block = audio.slice(start, end).replace(/\/\/[^\n]*/g, '')
    const entries = {}
    for (const [, key, file] of block.matchAll(/(\w+):\s*'([\w.]+\.wav)'/g)) entries[key] = file
    return entries
  }
  ...
  it('emits filenames that AGREE with the shell MUSIC + TUNES manifests', () => {
    ...
    // the one-shot TUNES, so OUTPUT_FILES mirrors BOTH manifests — whole, with no
    // carve-out. sw8-14 closed the last one (finishGround).
    const audio = readFileSync(join(repoRoot, 'src', 'shell', 'audio.ts'), 'utf8')
    const music = scrapeManifest(audio, 'export const MUSIC =')
    const tunes = scrapeManifest(audio, 'export const TUNES =')
    ...
    expect(tunes).toEqual({
      ...
      descent: 'descent.wav',
      finishGround: 'finish_ground.wav', // sw7-18 shipped it; sw8-14 makes it bakeable
      themeB: 'theme_b.wav', // sw8-12
    })
    // Every manifest entry, on both sides, with nothing destructured away. A tune
    // the game fetches but the bake cannot produce is an asset that exists only in
    // the bucket — one wipe from silence, and `just deploy-assets` cannot restore it.
    expect(OUTPUT_FILES).toEqual({ ...music, ...tunes })
  })

  it('scrapes each manifest WITHIN its own literal — the bound is real, not a count', () => {
    // Guards the guard. If the bound slipped, MUSIC's scrape would run on down the
    // file and swallow the TUNES entries, and the assertion above would start
    // "agreeing" about a manifest that does not exist. Cheap to state, and it is
    // the exact failure the old count-based helper shipped.
    const audio = readFileSync(join(repoRoot, 'src', 'shell', 'audio.ts'), 'utf8')
    const music = scrapeManifest(audio, 'export const MUSIC =')

    expect(Object.keys(music)).toHaveLength(4)
    expect(music).not.toHaveProperty('cantina')
    expect(Object.values(music)).not.toContain('finish_ground.wav')
  })
...
describe('sw7-8 — every one-shot tune bakes to real audio, not silence', () => {
  // EVERY tune in the catalogue, not a hand-kept subset. sw8-12 added themeB to
  // TUNE_SPEC and OUTPUT_FILES but not to this list, so a silent or clipped
  // theme_b.wav would have shipped under a green suite for exactly the reason the
  // header above gives — the gap this gate exists to close, reopened one entry at
  // a time. sw8-14 seats both the new finishGround and the missed themeB.
  const TUNES = ['deathKnell', 'cantina', 'finale', 'bensTheme', 'descent', 'themeB', 'finishGround']

  // ROM-derived duration floors (seconds), at ~80% of the measured render: the
  // knell is 48 accelerating tolls (~0.44 s); the others are full tunes. Measured
  // via renderVoice over the generated streams — REB is 16.03 s, THB 8.65 s.
  const MIN_SECONDS = {
    deathKnell: 0.3,
    cantina: 20,
    finale: 10,
