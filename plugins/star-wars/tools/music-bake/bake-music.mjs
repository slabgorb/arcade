...
// Everything bakeable: the four looping tracks plus the one-shot tunes (sw7-8,
// widened by sw8-12 and sw8-14) — same segment shape, same render path, same
// output directory.
...
export const OUTPUT_FILES = {
  ...
  // sw7-8 — the one-shot tunes (audio.ts TUNES):
  deathKnell: 'death_knell.wav',
  cantina: 'cantina.wav',
  finale: 'finale.wav',
  bensTheme: 'bens_theme.wav',
  descent: 'descent.wav',
  themeB: 'theme_b.wav', // sw8-12 — PMTHB split out of the space track
  finishGround: 'finish_ground.wav', // sw8-14 — PMREB split out of the towers track
}
 ...
 * order WSMAIN fires them.
 *
 * Every entry in the catalogue is single-segment today: the two multi-segment tracks
 * were split apart once the core could cue their second half at the ROM's own moment
 * (space lost THB at sw8-12, towers lost REB at sw8-14 — both are one-shot TUNES now).
 * The loop below still handles several segments, because that is what the ROM's
 * `.TUNE` sequences are shaped like and a future track may need it again.
 */
