...
// WSMAIN.MAC:
//
//   space   PMTH5 :1430 ";THEME MUSIC"                 (PMTHB is its own one-shot now — below)
//   towers  PM4TH :1636 ";BATTLE MUSIC IN FOURTHS"     (PMREB is its own one-shot now — below)
...
//
// ⚠ THB no longer rides the space track (sw8-12): the ROM fires PMTH5 at PH.TIM
// 40 (2s) and PMTHB at 200 (10s) — separate milestones with real SILENCE between
// theme end (~8.3s) and theme B. Concatenating them landed THB ~1.67s early, and
// the core's 10s `themeB` cue would then double-play it. THB is a one-shot below.
//
// ⚠ REB no longer rides the towers track (sw8-14), for exactly the same reason.
// PM4TH is a PHASE-ENTRY cue (PHIGD, WSMAIN.MAC:1636, with PH.TIM just zeroed);
// PMREB is a TIMED MILESTONE cue fired from the per-frame ground handler at
...
const TRACK_SPEC = {
  space: [{ tune: 'TH5', tuneIndices: [27, 28, 29, 30] }],
  towers: [{ tune: 'SW4', tuneIndices: [31, 32, 33, 34] }],
  trench: [{ tune: 'RR', tuneIndices: [23, 24, 25, 26] }],
  imperialMarch: [{ tune: 'DAR', tuneIndices: [43, 44, 45, 46] }],
}

// ── the one-shot tunes (sw7-8, audit U-010..U-014) ───────────────────────────
//
...
//   themeB       PMTHB WSMAIN.MAC:1435 ";THEME B FOLLOWS MAIN THEME" (space PH.TIM 200 — sw8-12)
//   finishGround PMREB WSMAIN.MAC:1673 ";FINISH GROUND WITH REBEL" (ground PH.TIM 14 — sw8-14)
const TUNE_SPEC = {
  ...
  themeB: { tune: 'THB', tuneIndices: [35, 36, 37, 38] },
  finishGround: { tune: 'REB', tuneIndices: [19, 20, 21, 22] },
}
...
// The one-shot tunes (sw7-8, U-010..U-014; sw8-12 themeB, sw8-14 finishGround) —
// same segment shape as TRACKS so bake-music's voice walker serves both.
// \`.CALL\`/\`.GOSUB\`/\`.RETURN\` are FLATTENED at generation time (see the
// generator), so these streams are pure 2-byte pairs like every track above.
export const TUNES = {
${Object.keys(TUNES)
  .map((t) => `  ${t}: {\n    segments: [\n${segLines(t, TUNES)}\n    ],\n  },`)
  .join('\n')}
