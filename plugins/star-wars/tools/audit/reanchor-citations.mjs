// Validate the audit's `ours` citations against the AUDIT COMMIT (td1-13).
//
// Every finding quotes one of our lines byte-for-byte. Since td1-13 the citation gate
// (`tests/audit/citations.test.ts`) re-opens each `ours` quote against the commit that
// recorded the audit — `git show 3580752:<file>` — not the working tree. The audit record is
...
// either it drifted off the immutable record (re-baseline it to its 3580752 text) or the
...
if (lost > 0) {
  console.log('LOST citations need a human: re-baseline the quote to its 3580752 text, or mark the finding `remediated_by`.')
}
