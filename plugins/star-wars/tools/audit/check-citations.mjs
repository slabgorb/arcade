...
// td1-13 — THE FREEZE. `ours` is read from the AUDIT COMMIT, never the working tree.
//
// Each finding's `ours.verbatim` IS the defect text as our code stood when it was audited.
// Reading it from the working tree meant the gate went red the moment any story fixed the very
// line the audit describes — so every fix story paid a hand re-pointing tax. We instead re-open
// `ours` against the commit that recorded the audit. That commit is immutable, so a later fix
// (or refactor) of the working tree can never redden the gate again. The ROM `source` side below
// stays LIVE against the 1983 assembler — that is where the audit's authority comes from, and it
// must still catch a mis-cited source line. Ported from tempest's tp1-22 (its 4232ed4). Reachable
// via the `audit/star-wars` tag; CI's fetch-depth: 0 keeps the blob alive.
...
// A tracked file as it stood at AUDIT_COMMIT, line-split — or `{ error }` if git cannot resolve
// it (the path did not exist at the audit). Cached per (repo, file); `git show` is invoked at
// most once per file. `git show` THROWS for an absent path, so the throw is caught and turned
// into a returned error the caller reports — the gate must never crash on a frozen citation it
// cannot resolve (td1-13 AC-4b).
 ...
 * @param opts.repoRoot   absolute path to the star-wars repo
...
export function checkFindings(findings, { repoRoot, sourceDir }) {
  ...
  for (const f of findings) {
      ...
      if (!LINKED_MODULES.includes(mod)) {
        errors.push(
          `${id}: cites ${f.source.file}, which never shipped ` +
            `(not in the WSROOT/WSOVLY/SNDAUX link set). Re-cite against the linked module.`,
        )
      } else if (sourceDir) {
    ...
    // authority comes from, and the 1983 source does not change. What we are giving up
    ...
    if (f.ours?.file && NODE_MODULES.test(f.ours.file)) {
      ...
      // because the rule was missing outright: the ROM does something we do not do
      // anywhere. Fixing that means ADDING code, so there is no historical quote to
      // preserve — `ours` was null when it was audited and null is still the truthful
      // answer. Demanding one would force a fix story to invent a citation for a line
      // that never diverged, which is exactly the kind of evidence this gate exists to
      // refuse.
      //
      // A fix story MAY still attach an `ours` to a NO_COUNTERPART, pointing at the code
      // that now implements the rule, and that is accepted too. Both are records of the
      // same fix, and neither is re-opened against the working tree. Two shapes, and
      // ONLY two. The exemption below is for a MISSING citation, not for an unchecked
      // one: before this guard, `remediated_by` + `NO_COUNTERPART` fell through with no
      // test at all, so an `ours` that was present but shapeless — no file, no line, no
      // quote, or not even an object — sailed past a gate whose whole job is to refuse
      // citations that cannot be re-opened. "May be null" and "may be anything" are not
      // the same permission.
      ...
      // THE FREEZE (td1-13). Re-open `ours` against AUDIT_COMMIT, not the working tree, and match
      // BY TEXT rather than by the stored line: earlier stories re-anchored `ours.line` to the
      // working tree, so the recorded row is no longer the audit-commit row. Mirroring
      // `reanchor-citations.mjs`, the quote is honoured if it appears anywhere in the file as it
      // stood at the audit — its line number is now decorative. A path that never existed at the
      // audit commit is a clear returned error, never a crash (AC-4b).
