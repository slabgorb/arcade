...
import type { GameState, Projectile, Enemy, Turret, Phase, TrenchObstacle, DyingTie, DestroyedShot, GroundDebris } from './state'
import {
  FIRE_INTERVAL,
  LASER_SWEEP_SECONDS,
  SPAWN_DISTANCE,
...
export function stepGame(stateIn: GameState, input: Input, dt: number): GameState {
  // The frame's YOKE AND VIEWPORT, shadowed onto the state ONCE (uf1-12) — every return
  // below spreads `...state`, so this one line reaches every exit path (and each of those
  // returns already writes these same three values, so nothing downstream changes).
  //
  // `computeStatus` rebuilds the gun's aim ray from `state` to derive C_PS, and it has to
  // be the SAME ray: the cabinet computes the laser hit and the sights bit from one
  ...
  if (state.mode === 'attract') {
    if (input.start) return openSelect(state, t)
  ...
  if (state.mode === 'select') {
    return stepSelect(state, input, dt, t)
  }
  ...
  // sw8-8 — ONE PILOT, ONE POINT. The gun fires from `shipPoint` in EVERY phase now. Space used
  // to special-case a drifting `spaceEye` here, on the sw7-16 promise that aim must follow view;
  // that promise is kept — but by moving the VIEW back onto the cockpit rather than dragging the
  // gun off it. `ST.UX` was never a camera (see the tombstone in gameRules.ts): the pilot does not
  // slide, so the muzzle, the camera, the C_PV pyramid, `homeShots`' target and the shield
  // hit-test are all the same origin again. That identity is the whole fix for sw8-8 — while the
  ...
  // Each phase runs its own combat, then `progress` checks the phase-end
  // condition and drops the run into the next phase once it is cleared. The
  // phase clock ticks INTO the stepper (ROM PHESP1: add, then compare, same
  // frame — sw8-11), so a stepper that warps internally (stepTrench's clearRun)
  // still restarts the entering phase's clock at 0 AFTER the tick.
  if (state.phase === 'surface')
    return finalizeFrame(
      state,
      finalizeScore(
        state,
        progress(stepSurface({ ...state, phaseTime: state.phaseTime + dt }, input, dt, common)),
      ),
    )
  if (state.phase === 'trench')
    return finalizeFrame(
      state,
      finalizeScore(state, stepTrench({ ...state, phaseTime: state.phaseTime + dt }, common, dt)),
    )
  ...
  // ROM order: the player can SEE it (C_PV — the gate's literal first test, `BITA
  // #C$PV/100 / BEQ` "NO SHOOTING GUNS IF PLAYER CANT SEE US", WSCPU.MAC:624-626; no
  // off-screen shooters, sw7-24 T5b) · is not in the AIM_AHEAD maneuver
  ...
  //
  // NO AIM CONE (sw8-9). This gate used to ALSO require C_AS ("player in the alien's sights",
  // a 12° nose cone) — an election the clone added and the cabinet does not have. WSCPU.MAC's
  // gate is the run at label 140$ (:622-633), which every preceding skip branches INTO, and it
  // tests C$PV, C$T9, the $800 floor and A$GLW — nothing else. C$AS is computed just above it
  // (:619-621) for the CHOREOGRAPHY to branch on (`.CUNTIL C$AS`) and is never read by the
  // gate. Keeping it made every shot a perfectly-lined-up one and starved the wave (measured:
  // 2 shots in 19 s), which is what read in play as fighters "sniping". C_PV still stands, so
  // an off-screen fighter — notably one behind the pilot after TCH1A1's authored turnover —
  // remains silent.
  ...
  while (frameAcc >= tickPeriod && catchup < MAX_CATCHUP_FRAMES) {
    ...
    tickedEnemies = tickedEnemies.map((e) => {
      ...
      const inView = (status & Status.C_PV) !== 0 // C$PV: no shooting guns if the player cannot see us
      ...
      let fired = false
      if (inView && !aimingAhead && inRange && notHit && (frame & params.fireMask) === 0) {
      ...
      //
      // C_AH is per-frame from the SAME rebuild, but it is cleared on EVERY fighter —
      // `computeStatus` has just read it above, and unlike `firedGun` it is written from
      // OUTSIDE this loop (the hit resolver, below). Riding on the `e.vm` ternary would
      // leave a VM-less fighter's flag set for the rest of the wave — a latch, where the
      // ROM has a one-frame event (uf1-3).
      const cleared = e.damaged ? { ...e, damaged: false } : e
      return e.vm ? { ...cleared, vm, firedGun: fired } : cleared
    })
    frameAcc -= tickPeriod
    catchup++
  }
  ...
  const movedEnemies = tickedEnemies
    ...
    // CLAMP to the play cube (sw8-9) — model §5 step 5, immediately after the step-4
    // position integration `applyManeuver` just did, which is where CPCHKL sits in the
    // cabinet's own per-frame order (WSCPU.MAC:799-811 / sub_8DE3). Each axis is pinned
    // INDEPENDENTLY, exactly as the ROM's `.IRPC .1,<XYZ>` loop does it — never by
    // vector length, which would drag the other two axes with it. Spread the fighter
    // rather than rebuilding it so optional fields (glow, vm, firedGun, damaged)
    // survive. `damaged` is the one that BITES: it is written by the hit resolver at
    // the END of a step and read by the NEXT decision tick, so at 60fps it has to
    // cross ~2 tick-less steps — and this map, the clamp below and the glow decay all
    // run on those steps. Rebuild the fighter here instead of spreading and C_AH stops
    // working in the shipped game (uf1-3). `tie-hit-status.test.ts` pins that.
    .map((e) => ({ ...e, pos: clampToPlayCube(e.pos) }))
  ...
  if (spawnTimer <= 0 && movedEnemies.length < WAVE_SIZE) {
    ...
    spawnCount += 1
    // Re-arm at ZERO (sw8-7): the ROM has no spawn cooldown — every space frame
    // tops the three alien slots back up (`LDA WV.LIV / CMPA #03 / IFLO / JSR
    // ADASHP`, WSMAIN.MAC:1450-1454), so a loss is replaced on the very next
    // step, one construct per step (ADASHP fills one slot per call). The
    // countdown itself survives as the fixtures' park lever (spawnTimer: 1e9).
    spawnTimer = 0
  }
  ...
  if (laserOn) {
    ...
    if (hitTie >= 0) {
      ...
      // ...and leave a brief red destroy-sparkle at the kill point (sw8-3) so the
      // interception READS on screen instead of the fireball just blinking out — the
      // silent-delete the playtest flagged. Same captured position as the cue above.
      spawnedBursts.push({ pos: [...enemyShots[hitShot].pos] as Vec3, age: 0 })
    }
  }
  const standingEnemies = enemies
    ...
    // `darthScored` is exactly "a hit CPHTSA PROCESSED" (it is gated on the glow window
    // the same way the ROM's `LDA A$GLW(X) / IFNE / RTS` gates the whole routine), so it
    // is also the right moment to raise C$AH — `CHSET C$AH` sits inside that same routine
    // at WSCPU.MAC:357. The next decision tick reads it into the VM and clears it (uf1-3).
    .map((e, i) => (darthScored.has(i) ? { ...e, glow: DARTH_GLOW_SECONDS, damaged: true } : e))
  ...
  // The downed-fireball sparkles get the exact same treatment (sw8-3): age this frame's
  // and older bursts, drop the finished ones, then add this frame's kills.
  ...
  let damage = 0
  // NO TIE-BODY COLLISION (sw8-9, model §9-3 / audit A-010). The cabinet has no
  // alien-body-vs-ship test anywhere in the alien code: WSCPU's COLLISION section
  // holds only CPHTSA ("SPACE LAZAR HIT ALIEN SHIP"), the player's laser decrementing
  // the alien's damage counter. The only thing that ever costs the pilot a shield is a
  // FIREBALL, handled directly below. A fighter that reaches the cockpit therefore
  // costs nothing and — the other half of the divergence — is NOT consumed: it keeps
  // flying its choreography, which is what "loiter" means. Untouched fighters leave
  // by being shot; the ROM's wave-end peel-away (sub_8B86) is not modelled here.
  const liveEnemies = standingEnemies
  ...
  // --- The PH.TIM music milestones (sw8-12) ----------------------------------
  // PHESP1 walks the space clock every game frame and fires each cue on an exact
  // compare (WSMAIN.MAC:1415-1444): the theme at PH.TIM 40 (2s) — PMDAR over
  // PMTH5 when GM.WAV >= 3 and odd, which is musicTrackFor's existing law — theme
  // B at 200 (10s, parity-blind: the CMPD chain tests PH.TIM only), and the
  // descent at 400 (20s), one second before the PH$SP2 warp the same chain ends
  // with (>= 420 = 21 s, ported by sw8-11 via phaseCleared). The phase itself
  // OPENS SILENT — PHISP1 plays nothing — so there is no edge cue anywhere: the
  // run start, the wave rollover, and the warp all cue no space music, and wave
  // 1's head-started clock (phaseTime seeded 1.95) fires the theme on the run's
  // first step, exactly the cabinet's "START A BIT AHEAD". Under float dt the
  ...
  const stepped = finalizeScore(
    ...
    progress({
      ...
      // The ROM PH.TIM tick (sw8-11): add, then let `progress` compare, in the
      // same frame (PHESP1's ADDD #1 precedes the milestone compares).
      phaseTime,
      projectiles,
      laserEdge,
      laserOn,
 ...
 */
/** attract + start opens the SELECT-A-DEATH-STAR picker (story sw9-2) — the
 *  ROM's PHISG1 -> PH$SDS edge (WSMAIN.MAC:1289/1017). Arms the countdown; the
 *  run itself begins only on the select->playing edge (`beginRun`), so no
 *  run-start cue fires yet and the RNG stays untouched (framing consumes no
 *  randomness). */
function openSelect(state: GameState, t: number): GameState {
  return finalizeFrame(state, {
    ...state,
    mode: 'select',
    select: { countdown: SELECT_COUNTDOWN_SECONDS, hover: null },
    t,
    events: [],
  })
}

/** Normalised-aim hit radius for the picker cursor (story sw9-2). Dev's tunable
 *  pick geometry — the ROM hit-tests fixed screen pixels + distance thresholds
 *  (WSMAIN.MAC:1082-1112) that don't survive the port to normalised aim (TEA's
 *  logged deviation): generous enough that a real aim on a choice registers,
 *  tight enough that DEATH_STAR_CHOICES' spread never overlaps. */
const SELECT_HIT_RADIUS = 0.18

/** Which Death Star (if any) the live aim is hovering — the closest choice
 *  within SELECT_HIT_RADIUS, or `null` off every choice. */
function hoverFromAim(aimX: number, aimY: number): number | null {
  let best: number | null = null
  let bestDist = SELECT_HIT_RADIUS
  for (let i = 0; i < DEATH_STAR_CHOICES.length; i++) {
    const [cx, cy] = DEATH_STAR_CHOICES[i].aim
...
/** The SELECT-A-DEATH-STAR picker step (story sw9-2) — the ROM's PHESDS. The
 *  countdown burns down by dt; the cursor hovers a Death Star off the live aim;
 *  a real fire on a hovered choice commits it EARLY, and the countdown running
 *  out with no choice defaults to EASY ("PLAYER DIDN'T DECIDE, START AT EASY",
 *  GM.WAV<-0). Both edges begin the run through `beginRun`, which is where the
 *  run-start cues fire. */
function stepSelect(state: GameState, input: Input, dt: number, t: number): GameState {
  const sel = state.select ?? { countdown: SELECT_COUNTDOWN_SECONDS, hover: null }
  const hover = hoverFromAim(input.aimX, input.aimY)
  if (input.fire && hover !== null) {
    const choice = DEATH_STAR_CHOICES[hover]
    return beginRun(state, choice.wave, choice.bonus)
  }
  const countdown = sel.countdown - dt
  if (countdown <= 0) {
    const easy = DEATH_STAR_CHOICES[0]
    return beginRun(state, easy.wave, easy.bonus)
  }
  return finalizeFrame(state, {
    ...state,
    t,
    select: { countdown, hover },
    events: [],
  })
}

/** Begin a fresh run at the chosen starting wave, banking its start bonus
 *  (story sw9-2) — the select->playing edge, the ROM's PH$SG2. Replaces the old
 *  attract+start `startRun`: the run now begins one screen later, after the
 *  picker (or its countdown timing out to EASY). Fires the run-start cues
 *  (player-spawn, Red Five reporting in) that used to fire directly off
 *  attract+start. NO music: the ROM's PHISP1 opens the space phase in SILENCE —
 *  the theme is the 2s PH.TIM milestone in the space stepper (sw8-12), which
 *  initialState's 1.95s head-start seed reaches on the run's first step. */
function beginRun(s: GameState, wave: number, bonus: number): GameState {
  return {
...
function stepSurface(state: GameState, input: Input, dt: number, common: StepCommon): GameState {
  ...
  if (laserOn) {
    ...
    if (hit >= 0) {
      killed.add(hit)
      if (turrets[hit].kind === 'bunker') {
        // Bunkers score a FLAT value (ROM TSCBNK = 200, SCRBNK) and are quota-neutral —
        // they never touch the escalating tower counter.
        score += TURRET_SCORE
      } else {
        // Towers ESCALATE (sw8-5): the tower killed now is the (phaseKills+1)th of the wave,
        // worth (phaseKills+1) × 200 — the ROM's running TWRMULT (SCRTWR, WSGAS.MAC:342). Only
        // one turret is hit per frame, so state.phaseKills (pre-increment) is the right index.
        // This is the SAME figure the HUD's "POINTS NEXT TOWER" readout shows for the next tower.
        score += nextTowerWorth(state.phaseKills)
        towerKills++
      }
...
function stepTrench(state: GameState, common: StepCommon, dt: number): GameState {
  ...
  if (detonates) {
    ...
    // NO space theme here (sw8-12): the ROM's wave rollover is musically SILENT —
...
//
// A run escalates through the three phases in order; each phase's own ROM end
// condition drops it into the next. The order and conditions are total over
// Phase, so the third phase can never be forgotten (the table is exhaustive).
...
/** Has this phase been cleared? Space is TIME-boxed (sw8-11): the ROM's PH.TIM
 * clock reaches 420 frames = 21 s and PHESP1 advances with IFHS — an unsigned
 * at-or-past compare, hence `>=` (WSMAIN.MAC:1441-1444); kills never enter the
 * decision, and the TIE supply refills to the end. The SURFACE ends by
...
function phaseCleared(s: GameState): boolean {
  switch (s.phase) {
    case 'space':
      return s.phaseTime >= SPACE_PHASE_END_S
    case 'surface':
...
function progress(s: GameState): GameState {
  ...
  // The descent tune (U-014) no longer rides this edge: PMDES is the space
  // PH.TIM 400 (20s) milestone in the space stepper (sw8-12, WSMAIN.MAC:1439),
  // one second before this warp — including wave 1's space -> trench dive, which
  // the old space->surface edge condition silently skipped.
...
export function enterPhase(s: GameState, phase: Phase): GameState {
  return {
    ...
    phase,
    phaseKills: 0,
    // Every ROM phase init stamps a fresh PH.TIM ("NEW PHASE TIMER JUST
    // STARTED", WSMAIN.MAC:2042) — the reset must be EXPLICIT here; the `...s`
    // spread would otherwise carry the spent clock into the new phase (sw8-11).
    phaseTime: 0,
    enemies: [],
    ...
    dyingTies: [],
    // A downed-fireball sparkle is wiped on phase entry too (sw8-3), like dyingTies.
    destroyedShots: [],
    groundDebris: [],
    turrets: [],
    ...
    // The surface turret machine keeps its own separate cadence, unchanged.
    spawnTimer: phase === 'surface' ? TURRET_SPAWN_INTERVAL : 0,
    enemyFireCooldown: ENEMY_FIRE_INTERVAL,
  }
}
...
/**
 * Pin a fighter inside the play cube (sw8-9) — the port of `CPCHKL` / `sub_8DE3`
 * (WSCPU.MAC:799-811, model §5 step 5). Each axis is compared and pinned on its
 * OWN, mirroring the ROM's `.IRPC .1,<XYZ>` macro loop: a position already inside
 * the cube comes back untouched, and a breach on one axis never disturbs the
 * other two. See PLAY_CUBE_MIN/MAX for the bounds and why they are raw ROM units.
 */
function clampToPlayCube(pos: Vec3): Vec3 {
  return [
    Math.max(PLAY_CUBE_MIN, Math.min(PLAY_CUBE_MAX, pos[0])),
    Math.max(PLAY_CUBE_MIN, Math.min(PLAY_CUBE_MAX, pos[1])),
    Math.max(PLAY_CUBE_MIN, Math.min(PLAY_CUBE_MAX, pos[2])),
  ]
}
...
/** A fresh TIE spawned far down −Z at the authentic depth, FACING THE PLAYER straight down
 * the depth axis. The LATERAL comes from the ROM TBG table walked in order by `spawnIndex`
 ...
 * offset across the field (world-x holds while depth shrinks, so |x|/depth WIDENS → the
