...
export function coachingFor(s: GameState): string | null {
  ...
  // `coaching` is a STORED field, re-derived only in `finalizeFrame`. The end-of-run hold used
  ...
  // sim.ts:227 is finalised now, and the death sites set gameOver alongside the mode
  // (sim.ts:763, :1267, :1535, :1708); this line is the half that does not depend on the mode.
