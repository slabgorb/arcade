...
import type { Vec3, Mat4 } from '@shared/math3d'
...
/**
 * The run lifecycle (story 8-6 framing, extended by sw9-2). The cabinet idles on
 * the attract/title screen; pressing start opens the SELECT-A-DEATH-STAR picker
 * (the ROM's PHISG1 -> PH$SDS edge, WSMAIN.MAC:1289/1017), which itself begins a
 * run (PH$SG2) once a Death Star is chosen or the countdown expires; then
 * game-over. The loop is attract -> select -> playing -> gameover -> attract.
 * The phases above happen WITHIN a 'playing' run; mode frames the run.
 */
export type Mode = 'attract' | 'select' | 'playing' | 'gameover'

/** The live SELECT-A-DEATH-STAR picker (story sw9-2) — the ROM's PHESDS. Armed
 *  on attract+start; `null` outside `mode === 'select'`. */
export interface SelectState {
  /** Seconds remaining before the ROM's "player didn't decide, start at EASY"
   *  default fires (PH.TIM, WSMAIN.MAC:1019/1046-1054). */
  countdown: number
  /** Index into `DEATH_STAR_CHOICES` the cursor is currently over, or `null`
   *  when the aim is off every choice. */
  hover: number | null
}

/** One Death Star on the picker — EASY/MEDIUM/HARD, mapping to the ROM's
 *  GM.WAV 0/2/4 (WSMAIN.MAC TDTH + PHESDS's cursor math; see the sw9-2
 *  story/session provenance ruling). */
export interface DeathStarChoice {
  label: string
  /** The clone's 1-based starting wave (romWave0 = wave - 1, the
   *  forceBonusForWave convention). */
  wave: number
  /** The start bonus banked when this Death Star is chosen — TSCBN[wave-1]. */
  bonus: number
  /** Normalised yoke position ([-1,1] per axis, matching Input.aimX/aimY) that
   *  hovers this choice. Dev's tunable pick geometry — the ROM hit-tests fixed
   *  screen pixels (WSMAIN.MAC:1082-1112) that don't survive the port to
   *  normalised aim (TEA's logged deviation). */
...
export interface Enemy {
  ...
  firedGun?: boolean
  /** True on the decision tick after this fighter took a PROCESSED hit — the ROM's
   * C$AH status bit. The equate calls it "NEARBY ALIEN HAS BEEN HIT"
   * (WSCPU.MAC:27), but the cabinet's only setter is `CPHTSA` (WSCPU.MAC:357),
   * which writes the bit back to the alien the laser hit — ";STATUS: THIS ALIEN
   * DAMAGED". It is SELF-damage; there is no proximity rule in the ROM (uf1-3).
   * Set behind the same no-double-jeopardy glow gate that scoring uses, because
   * CPHTSA returns before the CHSET when the alien is already glowing. Read by
   * `computeStatus` (tie-status.ts) to feed the choreography VM.
   * A per-frame flag like `firedGun`: the ROM's A$CHST rebuild keeps only C$PV +
   * the random bits (WSCPU.MAC:532-536), so it is cleared on the tick that reads
   * it — on EVERY fighter, not only VM-carrying ones, or it would latch.
   * Optional — every fighter that has not just been shot omits it. */
  damaged?: boolean
...
/** A fireball caught the instant the player shoots it down: a brief red sparkle drawn
 *  at the kill point that ages and expires, so a downed fireball leaves the cabinet
 *  afterimage instead of the silent-delete the playtest flagged (sw8-3, epic sw8
 *  observation #8). Purely a render cue — no collision, never fires — the exact shape
 *  of DyingTie. */
export interface DestroyedShot {
  /** World-space position where the fireball was destroyed. */
  pos: Vec3
  /** Seconds since the kill; the shell fades the sparkle by this and the sim drops the
   * entry once it passes FIREBALL_DESTROY_BURST_SECONDS. */
  age: number
}
...
// The genuine selection bonus is modelled below (story sw9-2, near TICK_HZ —
// SELECT_COUNTDOWN_SECONDS needs the game-frame timebase), in its correct home
// — the SELECT-A-DEATH-STAR picker's one-time start bonus.
...
// (sw8-7) SPAWN_INTERVAL is RETIRED — the ROM has no spawn timer. The space
// phase tops its three alien slots up every frame (`LDA WV.LIV / CMPA #03 /
// IFLO / JSR ADASHP`, WSMAIN.MAC:1450-1454); the spawner's countdown survives
// only as a fixture park lever, seeded and re-armed at 0 (sim.ts).
...
// --- SELECT-A-DEATH-STAR difficulty picker (sw9-2) ---------------------------

/** The countdown before the picker times out and defaults to EASY — the ROM's
 *  PH.TIM init (`LDD #0100`, WSMAIN.MAC:1019) = 256 game frames, in seconds via
 *  the shared game-frame timebase (TICK_HZ). Duration only — the displayed 8->0
 *  digit (`PH.TIM/32`) is a shell detail, not pinned here (sw9-2 deviation). */
export const SELECT_COUNTDOWN_SECONDS = 0x100 / TICK_HZ

/** The full ROM `TSCBN` start-bonus table (WSGAS.MAC:526-530), packed-BCD digit
 *  pairs, indexed by the 0-based ROM wave `GM.WAV`. `SCRWAV` (WSGAS.MAC:327-343)
 *  banks `TSCBN[GM.WAV]` once, as a pure function of the STARTING wave. Indices
 *  1 and 3 (200,000 / 600,000) are present in the ROM table but UNREACHABLE
 *  through the three-star picker below — see `DEATH_STAR_CHOICES` and the sw9-2
 *  session Delivery Findings. */
export const START_WAVE_BONUS: readonly number[] = [0, 200_000, 400_000, 600_000, 800_000]

/** The three shipped Death Stars (WSMAIN.MAC's `TDTH` table has 3 active rows;
 *  two more are commented out — an authentic-but-disabled five-star picker, out
 *  of this story's scope). `PHESDS`'s cursor math (`SUBD #TDTH / LSRB`) maps
 *  them to `GM.WAV` 0/2/4 — EASY/MEDIUM/HARD (TCMES.MAC:585-587) — so the
 *  reachable clone waves are 1/3/5 and the reachable bonuses are
 *  `START_WAVE_BONUS[0/2/4]` = {0, 400,000, 800,000}. */
export const DEATH_STAR_CHOICES: readonly DeathStarChoice[] = [
  { label: 'EASY', wave: 1, bonus: START_WAVE_BONUS[0], aim: [-0.5, -0.3] },
  { label: 'MEDIUM', wave: 3, bonus: START_WAVE_BONUS[2], aim: [0, -0.3] },
  { label: 'HARD', wave: 5, bonus: START_WAVE_BONUS[4], aim: [0.5, -0.3] },
]
...
/** How long a shot-down fireball's red destroy-sparkle lingers before the sim drops it
 *  (sw8-3). A cabinet-FEEL readability value, NOT a ROM port: the ROM silently removes
 *  an intercepted fireball, so this brief flash is a deliberate legibility add (epic
 *  sw8 observation #8). Sized just past the enemy muzzle-flash (~0.1 s) so a kill reads
 *  as a distinct pop rather than a blink. */
export const FIREBALL_DESTROY_BURST_SECONDS = 0.2
...
/**
 * The PLAY CUBE (sw8-9) — the ROM's containment bound, ported from `CPCHKL`
 * (`sub_8DE3`, WSCPU.MAC:799-811). Every game frame, after position integration,
 * the cabinet walks all three axes of the alien's location (`.IRPC .1,<XYZ>`) and
 * pins each one independently:
 *
 *     CMPA #7D / LDD #7CFF   ;KEEP POSITIVE DISTANCE IN RANGE
 *     CMPA #82 / LDD #8300   ;KEEP NEGATIVE DISTANCE IN RANGE
 *
 * The compares are on the HIGH BYTE, so the effective range is [$8300, $7CFF] —
 * exactly what the flight-AI model records (§5.3 "Position clamp"). Raw ROM units:
 * this repo's world metric is 16-bit ROM units with $4000 = 1.0, so ROM distances
 * port UNSCALED (see CLAUDE.md; TIE_NEAR_BOUND above sets the same precedent).
 *
 * This is CONTAINMENT, not choreography. The bound is symmetric about the pilot,
 * so it does not (and in the cabinet does not) stop a fighter passing behind the
 * eye — it only stops one escaping the cube. Measured on this sim, a fighter's
 * whole natural excursion (peak ~22.8k behind) sits well inside it, so the clamp
 * is a backstop that never fires during ordinary flight.
 */
export const PLAY_CUBE_MAX = 0x7cff // +31,999 — the ROM's positive pin
export const PLAY_CUBE_MIN = 0x8300 - 0x10000 // -32,000 — the ROM's $8300 as a signed 16-bit word
...
// trench). Each phase ends its own ROM-recovered way: the space phase is
// TIME-boxed on the PH.TIM clock (sw8-11, below), the surface ends by
// traversal (sw7-18 / D-019), and the trench ends at the exhaust port.

/**
 * Seconds of space combat before the run dives to the Death Star — the ROM's
 * PH.TIM time-box (sw8-11). PHESP1 adds 1 to PH.TIM every game frame
 * (WSMAIN.MAC:1415-1417) and advances to PH$SP2 ("GO AHEAD AND DESCEND") at
 * `CMPD #2+7+1+9+1+1*20.` with IFHS (WSMAIN.MAC:1441-1444). The `.MAC` sources
 * evaluate left-to-right, so that is (2+7+1+9+1+1)*20 = 420 frames — 21 s at
 * the 20 Hz game frame (the sibling `CMPD #2*20.  ;2 SECONDS` comments at
 * WSMAIN.MAC:2064/2096/2128 ratify both the arithmetic and the rate). There is
 * NO kill count in the ROM's phase-end path: TIEs refill throughout
 * (WV.LIV < 3 -> ADASHP, WSMAIN.MAC:1450-1454, ported by sw8-7) and kills only
 * score. This retires the invented 6-kill SPACE_WAVE_QUOTA.
 */
export const SPACE_PHASE_END_S = 21

/**
 * Head start on the space clock for the run's FIRST wave (sw8-11): while
 * SC.FWV == 0 ("0==>STILL ON FIRST WAVE", WSGLOB.MAC:760) PHISP1 seeds
 * PH.TIM = 2*20.-1 = 39 frames — "THEN START A BIT AHEAD"
 * (WSMAIN.MAC:1386-1390) — so wave 1's box runs ~19.05 s and its theme music
 * fires almost immediately. 39 / 20 Hz = 1.95 s.
 */
export const SPACE_PHASE_HEAD_START_S = 1.95
...
/** The ROM tower-score STEP (`TSCTWR` = .BYTE 00,02,00 = 200, WSGAS.MAC:520): the Nth
 *  tower a pilot destroys on a surface wave is worth N × this. The 1983 cabinet seeds
 *  `TWRMULT = TSCTWR` on ground entry (`IGRND`, WSGRND.MAC:697 "SETUP PROGRESSIVE SCORE
 *  TO FIRST VALUE") and adds `TSCTWR` per tower (`SCRTWR`, WSGAS.MAC:342-360). Equal to
 *  the flat `TURRET_SCORE` (the bunker/legacy value, `TSCBNK`), which is why tower #1 is
 *  unchanged at 200 and only towers #2+ escalate (400, 600, …). (sw8-5) */
export const TOWER_SCORE_STEP = 200

/** Points the NEXT surface tower is worth: `(towers already destroyed this phase + 1) ×
 *  TOWER_SCORE_STEP`. `phaseKills` counts towers only — bunkers are quota-neutral — so it
 *  IS the escalation index. This one value is both what the sim banks on a tower kill and
 *  what the HUD's "XXXXX POINTS NEXT TOWER" readout shows: one law, no on-screen lie
 *  (sw8-5; ROM `TWRMULT`, WSMAIN.MAC:3489-3506). */
export function nextTowerWorth(phaseKills: number): number {
  return (phaseKills + 1) * TOWER_SCORE_STEP
}

export interface GameState {
  /** Run lifecycle: attract/title, the SELECT-A-DEATH-STAR picker, an active
   *  run, or the game-over screen. */
  mode: Mode
  /** The live SELECT-A-DEATH-STAR picker (story sw9-2), or `null` outside
   *  `mode === 'select'`. */
  ...
  aimX: number
  aimY: number
  /** The viewport aspect (width / height) the frame was projected with — the
   *  shell's own `Input.aspect` (shell/input.ts), carried onto the state by
  ...
  /** Enemies destroyed in the CURRENT phase (scoring/HUD); resets to 0 on the
   * transition into the next phase. Kills do NOT end a phase (sw8-11): the
   * space phase is time-boxed on `phaseTime`, the surface ends by traversal. */
  phaseKills: number
  /** Seconds since the current phase began — the ROM's PH.TIM phase clock
   * (sw8-11). Accumulates dt every playing step, resets to 0 on every phase
   * entry (each ROM PHIxxx stamps PH.TIM — "NEW PHASE TIMER JUST STARTED",
   * WSMAIN.MAC:2042), and is seeded to SPACE_PHASE_HEAD_START_S by
   * `initialState` (the run start is the ROM's SC.FWV==0 first-wave case).
   * The space phase advances when this reaches SPACE_PHASE_END_S. */
  phaseTime: number
  ...
  dyingTies: DyingTie[]
  /** Fireballs the player has shot down this frame or recently, playing their brief red
   * destroy-sparkle (sw8-3). Each ages by dt and is dropped past
   * FIREBALL_DESTROY_BURST_SECONDS; wiped on phase entry like dyingTies. */
  destroyedShots: DestroyedShot[]
...
export function initialState(seed = 1983): GameState {
  return {
    ...
    // `enterPhase`, which restarts the clock at 0.
    phaseTime: SPACE_PHASE_HEAD_START_S,
    projectiles: [],
    laserEdge: 0,
    laserOn: false,
    enemies: [],
    ...
    firePrev: false,
    fireCooldown: 0,
    // Zero, not an interval: the wave opens deploying immediately — NWNSHP fills
    // every alien slot at phase init (WSCPU.MAC:969; sw8-7). Positive values park
    // the spawner (the test fixtures' quiet-sky lever).
    spawnTimer: 0,
    spawnCount: 0,
