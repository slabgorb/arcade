...
// @shared/audio in SH2-16 and adopted here in SH2-17. This module keeps only
...
export const TUNES = {
  ...
  themeB: 'theme_b.wav', // PMTHB — the space PH.TIM 200 (10s) milestone (sw8-12)
} as const
...
// tools/speech-bake/speech-data.mjs. EVERY key carries exactly one marker: [wired]
// if the core cues it (13 — the SpeechLine union in core/events.ts), [deferred] if
// nothing cues it yet (10). The markers were stale until uf1-5: 9 wired lines read
// as unwired. tests/shell/audio.test.ts now pins the marker set against the union,
// so wiring a line without re-marking it here fails the suite. The [deferred] ten
// are baked and serve 200 from R2 — dead weight, not broken assets; the mechanism
// each one waits on is inventoried under DEFERRED in core/events.ts.
export const SPEECH = {
  ...
  remember: 'remember.wav', // [wired] game over, first farewell word
  imOnTheLeader: 'i_m_on_the_leader.wav', // [deferred] Darth first sighted, SP.WAV even
  theForceIsStrongInThisOne: 'the_force_is_strong_in_this_one.wav', // [wired] trench timer 22, human EVEN wave
  redFiveImGoingIn: 'red_five_i_m_going_in.wav', // [wired] entering the surface, leads SIZ
  r2TryAndIncreaseThePower: 'r2_try_and_increase_the_power.wav', // [deferred] shield ladder, 2 left
  youreAllClearKid: 'you_re_all_clear_kid.wav', // [wired] trench timer 24, human ODD wave
  letGoLuke: 'let_go_luke.wav', // [wired] trench timer 16, human EVEN wave
  vaderBreathing: 'vader_breathing.wav', // [deferred] brackets the Darth lines + SPKSTR
  yahoo: 'yahoo.wav', // [deferred] leads SPKYAU; we cue only its second half
  iHaveYouNow: 'i_have_you_now.wav', // [deferred] Darth first sighted, SP.WAV odd
  stayInAttackFormation: 'stay_in_attack_formation.wav', // [deferred] Darth first sighted, SP.WAV even
  theForceWillBeWithYou: 'the_force_will_be_with_you.wav', // [wired] game over, 2nd of the farewell
  always: 'always.wav', // [wired] game over, 3rd of the farewell
  r2Scream: 'r2_scream.wav', // [wired] exhaust port missed and the run survives
  tieFighter: 'tie_fighter.wav', // [deferred] TIE pass-by (SPKELE) — audit U-019
  imHitButNotBadR2SeeWhatYouCanDoWithIt: 'i_m_hit_but_not_bad_r2_see_what_you_can_do_with_it.wav', // [deferred] first gun hit
  iveLostR2: 'i_ve_lost_r2.wav', // [deferred] shield ladder, 0 left
  iCantShakeHim: 'i_can_t_shake_him.wav', // [deferred] pursuit countdown
  lukeTrustMe: 'luke_trust_me.wav', // [wired] trench timer 16, human ODD wave
} as const
