...
// shared font). This story wires star-wars onto them: an Escape keydown EDGE
...
//   3. resolution — both subpaths resolve with the expected exports (originally a
//                   DEP-PIN contract against star-wars' ≥ v0.9.0 pin; since the monorepo
//                   migration the library is in-repo, so it is the same standing contract
//                   read through the `@shared` alias — it fails if src/shared drops an
//                   export or the alias stops resolving).
...
describe('SH2-14 — star-wars adopts @shared/pause + /esc-overlay (AC-1, AC-2)', () => {
  ...
  it('a src module imports the shared esc-overlay', () => {
    expect(
      ...
      'no src file imports @shared/esc-overlay — star-wars draws no pause overlay',
    ).not.toHaveLength(0)
  })
