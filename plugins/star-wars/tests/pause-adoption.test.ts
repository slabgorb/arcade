...
//
// Story SH2-14 (epic SH2) — RED phase (Furiosa / TEA). star-wars GAINS a pause:
// today only battlezone pauses, and SH2-12 published the shared mechanism as two
// shared-library subpaths — PURE `@shared/pause` (INITIAL_PAUSED / isPauseKey /
...
// shared font). This story wired star-wars onto them: an Escape keydown EDGE
// toggles pause, the frozen-frame gate holds the sim, and drawEscOverlay draws
// over the held cockpit — with star-wars supplying its OWN keybind card + colour
...
// (drawControlsOverlay, not drawEscOverlay). star-wars drops its drawEscOverlay
...
//   3. resolution — both subpaths resolve with the expected exports. This was a
//                   DEP-PIN contract (star-wars pinned @arcade/shared >= v0.9.0);
//                   post-monorepo it is an ALIAS-resolution contract against
//                   src/shared. The exports it demands are unchanged, and it still
//                   fails if either subpath loses them.
...
describe('SH2-14 — star-wars adopts @shared/pause + /esc-overlay (AC-1, AC-2)', () => {
  it('a src module imports the shared pause gate', () => {
    expect(
      importersOf(PAUSE_IMPORT),
      'no src file imports @shared/pause — star-wars has not wired the pause gate',
    ).not.toHaveLength(0)
  })
  ...
  it('a src module imports the shared controls overlay (sa1-5: owns pause chrome)', () => {
    expect(
      ...
      'no src file imports @shared/controls-overlay — star-wars draws no pause overlay',
    ).not.toHaveLength(0)
  })
