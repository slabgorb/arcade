...
import { beginRunAt } from '../support/select' // sw9-2: wave-1 run begins via the SELECT picker
 ...
 *  wave clears to `wave`, and the entered wave's theme is `musicTrackFor('space',
 *  wave)`.
 *
 *  sw8-12 re-seat: the theme is no longer an edge cue — the ROM's rollover is
 *  SILENT and the theme fires at the new wave's 2s PH.TIM milestone
 *  (WSMAIN.MAC:1418-1430). This helper is deliberately MOMENT-agnostic: it takes
 *  whatever the rollover frame cues, and if that is nothing (the ROM behaviour),
 *  parks the sky and flies to the milestone. The parity LAW this suite pins is
 *  the same at either moment; a half-fix that silences the edge without wiring
 *  the milestone leaves `tracks` empty and every case red. */
function musicEnteringWave(seed: number, wave: number): { out: GameState; tracks: string[] } {
  ...
  const tracks: string[] = [...musicTracks(out)]
  let s: GameState = { ...out, enemies: [], enemyShots: [], spawnTimer: 1e9 }
  // 2s at 1/60 steps ≈ 120 frames; 140 bounds the fly-to-milestone loop safely.
  for (let f = 0; f < 140 && tracks.length === 0; f++) {
...
describe('U-005 · Imperial March replaces the space theme on ROM waves {4,6,8,...} (WSMAIN:1421)', () => {
  ...
  it('wave 1 hears the PLAIN space theme at its head-started milestone (odd but < 4)', () => {
    // sw9-2: attract+start opens the SELECT picker; the wave-1 run begins when
    // EASY is picked (choice 0 = GM.WAV 0 = wave 1). sw8-12: the run-start edge
    // is SILENT — the theme fires at the 2s milestone, head-started to ~0.05s of
    // wall time on wave 1 (phaseTime seeded 1.95, WSMAIN.MAC:1386-1390). Same
    // moment-agnostic collection as musicEnteringWave.
    const out = beginRunAt(0, 1)
    expect(out.wave).toBe(1)
    const tracks: string[] = [...musicTracks(out)]
    let s: GameState = { ...out, enemies: [], enemyShots: [], spawnTimer: 1e9 }
    for (let f = 0; f < 10 && tracks.length === 0; f++) {
      s = stepGame(s, NO_INPUT, DT)
      tracks.push(...musicTracks(s))
    }
    expect(tracks).toContain('space')
