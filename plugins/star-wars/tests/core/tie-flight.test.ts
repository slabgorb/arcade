...
function followFirstTie(seed: number, steps: number, dt = 0.05): Sample[] {
  ...
  for (let i = 0; i < 40 && s.enemies.length === 0; i++) {
    s = stepGame(s, NO_INPUT, 0.375) // literal seconds (sw8-7 decoupled SPAWN_INTERVAL)
  }
...
describe('Story 9-2 — existing collision & motion contracts are unaffected (AC4)', () => {
  ...
  // sw8-9 INVERTED this contract (it read "still costs a shield and is removed"). Story 9-2's AC4
  // asks that the FLIGHT MODEL leave the collision contract alone — which it still does; what
  // changed is the contract itself. The ROM has no TIE-body collision (model §9-3, audit A-010),
  // so a fighter that reaches the pilot loiters there. The AC4 intent is preserved by asserting
  // the flight model does not perturb the new contract either.
  it('a TIE reaching the cockpit costs no shield and is not removed — it loiters (model §9-3)', () => {
    const base = initialState(1983)
    // Park the spawner (sw8-7: refills are instant now, so an unparked step would add a newcomer
    // and the length assertion would count it instead).
    const s = stepGame(
      { ...base, enemies: [tieAt([0, 0, 0])], spawnTimer: 1e9 },
      NO_INPUT,
      0.001,
    )
