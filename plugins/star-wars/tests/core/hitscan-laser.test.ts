...
import { length, sub, add, scale, normalize, type Vec3 } from '@shared/math3d'
 ...
 * a fresh obstacle field (`sim.ts:1204`), so both overrides are load-bearing: without them the
 ...
 * THROTTLE (`altitude += aimY · ALTITUDE_RATE · dt`, sim.ts:520). Aiming DOWN at a floor-level
...
describe('sw7-17 — enemy fire is still a real travelling object', () => {
  ...
  // travelling objects; only the player's laser is hitscan. `advance()` (sim.ts:1290) is shared
  ...
  it('a space TIE fireball still HOMES by the 7/8-per-tick decay (sw4-2 stands)', () => {
    // The space fireball's motion is not `advance` at all but `homeShots` (sim.ts:1311) — the
...
describe('sw7-17 — the trench beam is clipped to 28,672 units forward (CLBLZ)', () => {
  ...
  it('NEVER destroys a square just OUTSIDE the clip, however long the pilot waits', () => {
    ...
    // POINT against a 90-unit sphere (`collides`, sim.ts:817), so whether it registers depends
