 ...
 *  maze is laid by the real phase transition (whatever mechanism rules the
 *  space end — the staging is valid under both the retired kill quota and the
 *  sw8-11 time-box, see tests/support/space-phase-end.ts). */
function enterSurface(seed: number, wave: number): GameState {
  let s: GameState = {
    ...initialState(seed),
    wave,
    phase: 'space',
    ...SPACE_PHASE_OVER, // at the space phase's end — forces the space→surface cross
    enemies: [],
    enemyShots: [],
  }
