...
// ported ROM time-box. This suite pins the WARP (what a phase edge carries and
// resets); WHEN space ends is sw8-11's own contract (space-phase-timebox.test.ts).
import {
  SPACE_PHASE_OVER,
  SPACE_PHASE_NOT_OVER,
  SPACE_PHASE_CLOSING_KILL,
} from '../support/space-phase-end'
...
describe('Wave progression — phase-clear counter', () => {
  ...
  it('defines a positive wave-scaled surface tower count', () => {
    // The space phase's end is TIME-boxed (sw8-11, space-phase-timebox.test.ts) —
    // no space quota constant remains to pin here. The surface count is
    // wave-scaled (mazeForWave().towerCount). Wave 3 (SQUARE) is the first
    // ground wave WITH towers — wave 1 has no ground phase (D-015) and wave 2
    // (BUNK) is bunkers-only, both 0.
    expect(towersForWave(3)).toBeGreaterThan(0)
  })

  it('counts kills in the phase kill counter', () => {
    const s0: GameState = {
      ...space(),
...
describe('Wave progression — space clears to surface (wave 2)', () => {
  it('stays in space while the phase is not yet over', () => {
    const s0: GameState = { ...spaceBeforeSurface(), ...SPACE_PHASE_NOT_OVER, enemies: [], enemyShots: [] }
    const s1 = stepGame(s0, NO_INPUT, 0.05)
    expect(s1.phase).toBe('space')
  })

  it('advances to surface once the space phase is over', () => {
    const s0: GameState = { ...spaceBeforeSurface(), ...SPACE_PHASE_OVER, enemies: [], enemyShots: [] }
    const s1 = crossFrom(s0, 'space')
    expect(s1.phase).toBe('surface')
  })

  it('the closing kill scores and carries score/lives forward into surface', () => {
    const s0: GameState = {
      ...spaceBeforeSurface(),
      ...SPACE_PHASE_CLOSING_KILL,
      score: 555,
...
describe('Wave progression — ordering, determinism & trench safety', () => {
  ...
  it('a finished run (game over) never advances phases', () => {
    const s0: GameState = { ...space(), ...SPACE_PHASE_OVER, lives: 0, gameOver: true }
