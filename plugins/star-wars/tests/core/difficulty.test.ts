...
//
// NOTE (sw8-7): the `spawnInterval` axis was RETIRED from this suite. The ROM
// has no spawn interval — the space phase keeps its three alien slots topped up
// every frame (WSMAIN.MAC:1450-1454), so a per-wave spawn-cadence ramp was an
// invention. The spawn contract lives in tie-spawn-cadence.test.ts; the fire
// axes below are untouched.
import { describe, it, expect } from 'vitest'
import { ENEMY_FIRE_INTERVAL } from '../../src/core/state'
...
describe('waveParams — escalates with the wave number (AC: difficulty ramp)', () => {
  // (sw8-7) "tightens spawn cadence on later waves" retired — see the header note.
