...
describe('Story sw2-1 — the retreat path and aim precision must survive the reach fix', () => {
  ...
  it('a straight-ahead shot does NOT hit an inbound TIE off to the side (reach must stay on the aim line)', () => {
    ...
    // This line used to read `expect(s.lives).toBe(STARTING_LIVES)` with the comment "and it never
    // rammed (stationary)". sw8-9 removed TIE-body collision, so a ram can no longer cost a shield
    // and that assertion stopped guarding anything; worse, it now FAILS for an unrelated reason —
