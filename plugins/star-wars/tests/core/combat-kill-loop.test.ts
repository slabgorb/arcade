...
// ported ROM time-box — this suite's subject is fire-vs-ram, not the phase gate.
...
describe('Story 8-16 — TIEs die to fire, never to ramming (phase end per sw8-11)', () => {
  it('shooting the final TIE across the closing frame is a clean clear — no shield lost', () => {
    ...
    // whichever mechanism rules, the clear must come from FIRE, costing nothing.
    let s = loneWave(tie, { wave: 2, ...SPACE_PHASE_CLOSING_KILL })
    const fire: Input = { ...aimAt(tie.pos), fire: true }
    for (let i = 0; i < 180 && s.phase === 'space'; i++) s = stepGame(s, fire, DT)
    expect(s.phase).toBe('surface') // the phase really closed
    expect(s.lives).toBe(STARTING_LIVES) // cleared by fire — no shield lost to a ram
  })

  it('a TIE sitting on the cockpit is not a kill, holds the phase, and now costs nothing', () => {
    // Mid-box, with a TIE already on the cockpit. THE SUBJECT IS UNCHANGED: a fighter at the
    // cockpit must never be counted as a kill or clear the wave — that is what this guards, and
    // sw8-9 does not touch it. What sw8-9 DID change is the third assertion: the shield. The ROM
    // has no alien-body-vs-ship collision (model §9-3 / audit A-010), so reaching the pilot costs
    // nothing at all; the line below read `STARTING_LIVES - 1` under the old ram contract.
    // 5 = one short of the OLD quota literal, kept as a literal so the counter's non-movement is
    // pinned exactly (sw8-11).
    const s0 = loneWave(tieStill([0, 0, 0]), { ...SPACE_PHASE_NOT_OVER, phaseKills: 5 })
    const s1 = stepGame(s0, { aimX: 0, aimY: 0, fire: false }, DT)
    expect(s1.phase).toBe('space') // not a kill — no wave clear
    expect(s1.lives).toBe(STARTING_LIVES) // and no shield: TIE bodies are harmless (sw8-9)
    expect(s1.phaseKills).toBe(5) // and it did not count as a kill
  })
})
