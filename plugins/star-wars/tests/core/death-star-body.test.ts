...
/** sw8-11: drive the approach through BOTH mechanisms at once — k kills of the
 *  old 6-kill quota alongside the equivalent clock (21·k/6 s of the ROM PH.TIM
 *  box), so these keep-behaviour guards stay green across the mechanism swap.
 *  Literals on purpose; the Partial cast is RED-transitional (exact once
 *  `phaseTime` lands in GameState). */
const spaceState = (k: number): GameState => ({
  ...initialState(1983),
  phase: 'space',
  phaseKills: k,
  ...({ phaseKills: k, phaseTime: (21 * k) / 6 } as Partial<GameState>),
})
...
describe('11-7 — Death Star body placement grows on approach (pure, from sim state)', () => {
  ...
  it('clamps growth at the quota — no overshoot past full approach', () => {
    ...
    if (!fn) return
    const atQuota = fn(spaceState(6))
    const beyond = fn(spaceState(11)) // beyond full approach under BOTH mechanisms
