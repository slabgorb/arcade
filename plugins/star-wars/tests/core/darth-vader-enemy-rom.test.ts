...
// the player-bolt loop (sim.ts:291-303) is a flat one-hit kill that always adds
...
//
// RETREAT ("flees at wave end", A-016): the space phase always ENDS on its own
// (sw8-11: the ROM PH.TIM time-box; formerly the 6-kill quota) and `enterPhase`
// wipes every surviving enemy — so an immortal Darth cannot deadlock the wave
// and "leaves at wave end" is architecturally automatic. A dedicated retreat test would be VACUOUS (green
...
describe('sw7-13 A-016 — the RTH wave slot spawns a live Darth', () => {
  it('a non-TIE fighter is spawned across the Darth-scheduled space waves', () => {
    ...
    // kind:'tie' (sim.ts:1366), so no Darth ever appears. Force a spawn every
