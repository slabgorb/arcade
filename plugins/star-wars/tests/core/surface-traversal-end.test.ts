...
// >> `loseShield` / `const lives` (:1206-1207) and the gated push (:1227-1232).
//
...
// ENTERED dead; the dispatcher (sim.ts:221) makes that unreachable, so gating on
...
// below it, while `loseShield` (:1206) is the ONE place damage becomes death.
...
// "still cues" cases, and disabling the gameover branch at sim.ts:221 reddens
...
describe('sw8-21 — a finishGround crossing on the death frame cues NOTHING', () => {
  ...
  it('silences it for a TURRET BOLT too — damage that lands BELOW the cue site', () => {
    ...
    // (:1206-1207) sees every source, whatever they are.
  ...
  it('a frame ENTERED dead never reaches the stepper, so the crossing never even runs', () => {
    ...
    // cue site would be a no-op: sim.ts:210 returns on the gameover branch before
