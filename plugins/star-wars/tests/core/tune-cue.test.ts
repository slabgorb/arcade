...
//   space PH.TIM 400 (20s)        WSMAIN.MAC:1439  "JSR PMDES" at space  descent
//     — a MILESTONE, not the      PH.TIM 400, 20 frames before the       (U-014)
//     edge (ported by sw8-12)     PH$SP2 descend flip at :1442
//
...
describe('tune cue — the finale fires when the Death Star detonates (U-012, WSMAIN.MAC:2179 PHIDX1)', () => {
  ...
  it('arming inside the window fires BOTH, knell before finale (the ROM call order)', () => {
    ...
    // pilot must be flying the floor himself.
    //
    // RE-SEATED for sw10-1's ±45° authentic lens (fixture re-derivation, not a lens-semantic
...
describe('tune cue — the descent is the 20s PH.TIM milestone, ONE second before the warp (U-014 / sw8-12, WSMAIN.MAC:1436-1444)', () => {
  // sw8-12 re-seat: the old test pinned the descent to the space→surface EDGE —
  // the "un-sequenced equivalent" this suite's own header called it. sw8-11's
  // phaseTime clock exists now, so the ROM moment is directly portable: PMDES
  // fires at PH.TIM 400 (20s), twenty frames before the PH$SP2 flip at 420 (21s).
  it('cues descent the step phaseTime crosses 20 — while STILL flying space, no warp this frame', () => {
    // WAVE 2 — wave 1 has no ground phase (D-015) but the milestone is phase-clock
    // driven either way. Sky parked so the 20s frame is deterministic.
    const staged = playing({
      phase: 'space', wave: 2, enemies: [], enemyShots: [], spawnTimer: 1e9, phaseTime: 19.99,
    })
    const s1 = stepGame(staged, NO_INPUT, DT)
    expect(s1.phase).toBe('space') // 20s is NOT the warp — that is 21s (sw8-11)
    expect(tunesOf(s1)).toContain('descent')
  })

  it('the space -> surface WARP edge no longer carries the descent — it rang a second earlier', () => {
    // Staged past the 20s milestone (already spent), so the next step is the
    // sw8-11 time-box warp itself. Only the towers loop opens the phase
    // (sw3-5's edge contract for the SURFACE is untouched by sw8-12).
    const staged = playing({
      phase: 'space', wave: 2, enemies: [], enemyShots: [], spawnTimer: 1e9, phaseTime: 20.99,
    })
    const s1 = stepGame(staged, NO_INPUT, DT)
  ...
  it('does NOT cue descent on a run start, nor on the port-kill warp to the next wave', () => {
    // Run start: attract -> SELECT picker (sw9-2) -> playing opens in SILENCE
    // (sw8-12) — certainly no descent. The picker interposes but emits no
    // descent tune either, so this negative holds on the attract+start step.
