...
import { initialState, SKIM_ALTITUDE, MIN_SKIM_ALTITUDE, PROJECTILE_TTL } from '../../src/core/state'
// sw8-11: end-of-space staging valid under both the retiring kill quota and the
// ported ROM time-box — this suite's subject is the event shapes, not the mechanism.
...
import { fireAtChoice } from '../support/select' // sw9-2: run begins on the SELECT→play edge
...
describe('event emission — space phase gameplay moments (AC1)', () => {
  ...
  it("emits 'player-death' (cause 'enemy') and spends a shield when a FIREBALL reaches the cockpit", () => {
    // RE-SEATED by sw8-9 (was: a TIE at the cockpit). This suite's subject is the EVENT — that
    // space emits `player-death` with `cause: 'enemy'` and spends a shield — not which object
    // delivers it. sw8-9 removes the TIE-BODY collision (model §9-3 / audit A-010: the ROM has no
  ...
  it("emits 'level-clear' (next: surface) when the space phase clears into the surface", () => {
  ...
  it("emits 'player-spawn' when a run begins from the SELECT picker (sw9-2)", () => {
    // sw9-2: attract+start opens the SELECT-A-DEATH-STAR screen; the run begins —
    // and the spawn cue fires — on the select→playing edge (picking a Death Star).
    const sel = stepGame(playing({ mode: 'attract' }), { ...NO_INPUT, start: true }, DT)
    expect(sel.mode).toBe('select')
    const out = fireAtChoice(sel, 0) // choice 0 = EASY
