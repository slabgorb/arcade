...
// ported ROM time-box — this suite's subject is the debounce, not the mechanism.
...
describe('S-016 — at most one shield lost per gauge-redraw cycle (WSGLOW.MAC BG1GLW / WSGAS.MAC DO1GAS)', () => {
  ...
  // REWRITTEN by sw8-9. This case used to read "caps across DIFFERENT sources — a TIE and a
  // fireball landing together cost ONE shield". sw8-9 removes the TIE-BODY collision (model §9-3 /
  // audit A-010: the ROM has no alien-body-vs-ship test), leaving space with exactly ONE damage
  // source, so "different sources" is no longer a thing the space phase can express — and left as
  // it was, the case would still pass while testing nothing the isolated-hit case below does not
  // already cover. Restated as the sw8-9 contract, which IS discriminating: the shield comes from
  // the fireball ALONE, and the proof is that the TIE is still on the board afterwards (the old
  // body collision both charged a shield AND deleted the fighter, so a regression that restored it
  // would empty `enemies` and fail here).
  it('a TIE sharing the frame with a fireball adds NOTHING — one shield, and the TIE is still there', () => {
    const s0 = hitFrame({ enemies: [tie(AT_COCKPIT)], enemyShots: [shot(AT_COCKPIT)] })
    const out = stepGame(s0, NO_INPUT, TICK)
...
describe('S-016 — the post-hit window survives a wave-internal phase change', () => {
  ...
  it('a fresh hit survives the space->surface clear through the real step (not just enterPhase)', () => {
    // Seed the space phase already at its end so this step advances to the surface,
    // with a hit stamped this same frame. Pre-fix, progress()->enterPhase nulled shieldHitAt here.
