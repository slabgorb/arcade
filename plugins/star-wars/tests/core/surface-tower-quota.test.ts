...
describe('sw3-3 — clearing every tower banks the 50,000 exactly once', () => {
  ...
  it('the clearing tower kill scores BOTH its escalated tower value and the 50,000 bonus', () => {
    // The last (16th, SQUARE) tower is alive; the player's beam kills it, meeting
    // the quota — score gets the 16th tower's ESCALATED worth (sw8-5) AND the
    ...
    // sw8-5 re-seat: towers escalate (Nth worth N * TSCTWR, TSCTWR = TURRET_SCORE = 200 —
    // pinned in surface-tower-escalation.test.ts). phaseKills was 15, so this is the 16th
    // tower: 16 * 200 = 3,200, banked alongside the 50,000 cleared-all bonus. Was
    // `TURRET_SCORE + SURFACE_CLEAR_BONUS` (a flat 200) before escalation.
    expect(s1.score).toBe(16 * TURRET_SCORE + SURFACE_CLEAR_BONUS) // 3,200 + 50,000 = 53,200
  })
  ...
  it('does NOT award the tower bonus on the space→surface transition', () => {
    ...
    const s0: GameState = {
      ...initialState(1983),
      wave: 2, // wave 2 DOES have a surface (D-015); the space phase clears into it
      phase: 'space',
      ...SPACE_PHASE_OVER, // at the space phase's end (valid under quota AND time-box)
      score: 0,
      enemies: [],
      enemyShots: [],
    }
