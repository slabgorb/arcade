...
describe('sw7-16 — the other phases do not move', () => {
  ...
  it('space still takes cockpit hits at the origin', () => {
    ...
    // it a guard.
    // RE-SEATED by sw8-9 from a TIE to a FIREBALL. The guard's subject is that space's cockpit
    // hit-test routes through `shipPoint` — not which object trips it. sw8-9 deletes the TIE-BODY
    // collision (model §9-3 / audit A-010), so a TIE can no longer trip ANY hit-test and the guard
    // would go permanently inert — the exact failure mode the comment above warns about. The
    // fireball arm of the same block reads the same `shipPoint(state)`, so the guard still bites.
    const s0: GameState = {
      ...enterPhase(initialState(1983), 'space'),
      mode: 'playing',
      altitude: MAX_SKIM_ALTITUDE,
