...
//     STARTING_LIVES, TIE_SCORE, PROJECTILE_TTL, FIRE_INTERVAL, WAVE_SIZE
//   (sw8-7: SPAWN_INTERVAL is no longer referenced — the step sizes below are
//   plain literal seconds, so the spawn-cadence rework can retire the constant
//   without touching this suite. The spawn contract: tie-spawn-cadence.test.ts.)
//
...
describe('Wave 1 — enemy spawning & movement', () => {
  it('the wave opens with an empty STATE — the spawner deploys through stepGame', () => {
    // (sw8-7 ruling) `initialState` stays enemy-empty as the fleet's fabrication
    // base; the ROM's frame-zero swirl is delivered by the spawner in the first
    // ticks (tie-spawn-cadence.test.ts pins that onset). This asserts the STATE
    // at rest, not the sky the player sees.
    expect(wave().enemies).toHaveLength(0)
  })

  it('TIE fighters deploy as the wave opens', () => {
    let s = wave()
    for (let i = 0; i < 16; i++) s = stepGame(s, NO_INPUT, 0.1875)
  ...
  it('spawns identically for a fixed seed (determinism)', () => {
    ...
    let b = wave(7)
    for (let i = 0; i < 20; i++) {
      a = stepGame(a, NO_INPUT, 0.75)
      b = stepGame(b, NO_INPUT, 0.75)
    }
...
describe('Wave 1 — collisions, scoring & lives', () => {
  ...
  // sw8-9 INVERTED this contract. It used to read "a TIE reaching the cockpit costs a life and is
  // removed" — the ram. The ROM has no alien-body-vs-ship test anywhere in the alien code (audit
  // A-010; the choreography scripts end in an infinite loiter, WSCPU.MAC:1387, and CPCHKL only
  // clamps the alien to the play cube), so a TIE that reaches the pilot costs NOTHING and stays on
  // the board. This is not a re-seat — the old sentence was the divergence, and it is now reversed.
  // The shield-loss coverage it used to provide lives on in the fireball case directly below.
  it('a TIE reaching the cockpit costs NOTHING and is NOT removed — it loiters (model §9-3)', () => {
    const base = wave()
    // Park the spawner (sw8-7: refills are instant now, so an unparked step would add a newcomer
    // and turn the length assertion into a count of the wrong fighters).
    const s0: GameState = { ...base, enemies: [tie([0, 0, 0])], spawnTimer: 1e9 }
    const s1 = stepGame(s0, NO_INPUT, 0.001)
    expect(s1.lives).toBe(base.lives)
    expect(s1.enemies).toHaveLength(1)
  })

  it('a FIREBALL reaching the cockpit still costs a life and is removed', () => {
    // The replacement coverage for the inverted test above: space still HAS a damage path, and
    // this is it. Without this, deleting the body collision would leave "reaching the cockpit
    // hurts" untested in this suite entirely.
    const base = wave()
    const s0: GameState = { ...base, enemyShots: [shot([0, 0, 0])], spawnTimer: 1e9 }
    const s1 = stepGame(s0, NO_INPUT, 0.001)
    expect(s1.lives).toBe(base.lives - 1)
    expect(s1.enemyShots).toHaveLength(0)
  })
