...
//
// ISOLATION (calibrated against the current frame, /tmp probe): at full
// approach (SPACE_PHASE_OVER) the Death Star projects to the CENTRAL disc (radius ≤ ~200 of
...
// sw8-11: closest-approach staging valid under both the retiring kill quota and
// the ported ROM time-box — this suite's subject is the picture, not the clock.
...
function spaceFrame(over: Partial<GameState> = {}): GameState {
  return {
    ...
    mode: 'playing',
    phase: 'space',
    ...SPACE_PHASE_OVER, // closest approach ⇒ biggest, easiest to read
    enemies: [],
    dyingTies: [],
    enemyShots: [],
