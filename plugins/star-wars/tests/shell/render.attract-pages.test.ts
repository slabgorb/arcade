...
//
// sw9-3 RE-SEAT (AC-2). This guard originally used "PULL TRIGGER TO START" as the
// banner sentinel — but that string is WRONG for the job. The cabinet draws MS.STR via
// VWCOIN on ALL FOUR attract phases (PHEBNR:1268 / PHEINS:702 / PHESCR:733 / PHEHIS:904),
// so the invitation is a PERSISTENT cross-page line, not a banner-only element — sw9-3
// AC-1 now draws it on every page (see render.attract-start-prompt.test.ts). The
// genuinely banner-only element is the STAR WARS title, drawn only by VWBNNR (banner
// phase). Sentinel re-seated to "STAR WARS", which preserves F5's real intent (no page
// silently renders the whole banner) without the false coupling to the invitation.
describe('sw7-10 F5 — no page silently falls through to the banner', () => {
  it('the non-banner pages do NOT draw the banner marquee title', () => {
    for (const page of ['instructions', 'scoring', 'hiscore'] as const) {
      font.calls.length = 0
      render(makeCtx(), attractOn(page), W, H, [])
      expect(has('STAR WARS'), `the ${page} page must not render the banner`).toBe(false)
    }
  })

  it('the banner page draws the marquee and NEITHER text page', () => {
    render(makeCtx(), attractOn('banner'), W, H, [])
    expect(has('STAR WARS')).toBe(true)
    expect(has('FLIGHT INSTRUCTIONS TO RED FIVE')).toBe(false)
    expect(has('SCORING')).toBe(false)
  })
})
