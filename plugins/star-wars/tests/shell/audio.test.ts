...
// The two sides of the [wired]/[deferred] contract, read as text (uf1-5): the
// SpeechLine union is a TYPE with no runtime form, and the markers are comments.
import eventsSrc from '../../src/core/events.ts?raw'
import audioSrc from '../../src/shell/audio.ts?raw'
...
// --- [wired]/[deferred] marker accuracy (uf1-5) ------------------------------
//
// The catalogue's markers are prose, and prose rots: before uf1-5 only 4 of the
// 13 core-cued lines carried [wired], so 9 correct wirings read as unwired, and
// the events.ts header still claimed 19 deferred lines when the true remainder
// was 10. These pin the two sides against each other so the next wiring cannot
// silently re-open the gap — a line moved into the union without re-marking it
// here fails, and so does the reverse.
describe('speech catalogue markers track the core cue set (uf1-5)', () => {
  // Slice to the blank line that terminates the union so the DEFERRED inventory
  // comment beneath it cannot leak into the scrape.
  const unionBlock = eventsSrc
    .slice(eventsSrc.indexOf('export type SpeechLine ='))
    .split('\n\n')[0]
  const CUED = [...unionBlock.matchAll(/\|\s*'([A-Za-z0-9]+)'/g)].map((m) => m[1])

  // The SPEECH object literal alone — sliced so the other .wav manifests in the
  // same module (SOUNDS, MUSIC, TUNES) cannot match.
  const speechStart = audioSrc.indexOf('export const SPEECH = {')
  const speechBlock = audioSrc.slice(speechStart, audioSrc.indexOf('} as const', speechStart))
  const marked = (marker: 'wired' | 'deferred'): string[] =>
    [...speechBlock.matchAll(/^\s*([A-Za-z0-9]+):\s*'[^']+',\s*\/\/\s*\[(wired|deferred)\]/gm)]
      .filter((m) => m[2] === marker)
      .map((m) => m[1])

  it('guards the guard — both scrapes found their real contents', () => {
    // If either regex silently broke, the equality checks below would pass
    // vacuously (two empty sets are equal).
    expect(CUED.length).toBe(13)
    expect(Object.keys(SPEECH).length).toBe(23)
    expect(marked('wired').length + marked('deferred').length).toBe(23)
  })

  it('every catalogue key carries exactly one marker', () => {
    expect([...marked('wired'), ...marked('deferred')].sort()).toEqual(Object.keys(SPEECH).sort())
  })

  it('[wired] is exactly the set of lines the core cues', () => {
    expect(marked('wired').sort()).toEqual([...CUED].sort())
  })

  it('the deferred remainder is the 10 that events.ts inventories', () => {
    expect(marked('deferred').length).toBe(10)
    // …and it is a true complement, not a coincidence of counts.
    expect(marked('deferred').sort()).toEqual(
      Object.keys(SPEECH)
        .filter((k) => !CUED.includes(k))
        .sort(),
    )
  })
})
