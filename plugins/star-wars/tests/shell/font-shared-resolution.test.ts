...
//
// RED-phase suite for Story SH2-5, originally the dependency-pin contract:
// star-wars had to consume the shared library at a ref whose exports map carried
// the /font subpath AND whose glyph table covered the audited star-wars character
// set — notably ',' (en-US toLocaleString grouping in formatScore and the
// force-bonus banner), the SH2-3 gap glyph absent from every tag up to v0.6.0. The
// pin at the time (v0.5.0) had no /font subpath at all, so this failed until Dev
// re-pinned (tag v0.7.0, cut at SH2-3's f9676be). Since the monorepo migration the
// library is in-repo, so this is the same glyph-coverage contract read through the
// `@shared` alias.
//
...
// is exactly what a static `import('@shared/font')` did to the
...
describe('SH2-5 — @shared/font resolves with the star-wars glyph set', () => {
