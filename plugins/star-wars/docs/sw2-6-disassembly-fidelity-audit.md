...
> **⚠ SUPERSEDED (uf1-5):** the Audio/music row's speech figure is stale. Music
> shipped (sw3-5), the trench voice timer shipped (sw3-4), and the core now cues
> **13 of 23** lines, not 4 — so the deferred remainder is 10. The row is left as
> written above, not rewritten; the live inventory (and what each of the 10
> remaining lines waits on) is the `DEFERRED` block in `src/core/events.ts`.
...
- **Only 4 of 23 baked speech lines cued.** `audio.ts` bakes all 23; the
  ...
  *Approximated.*
  > **⚠ SUPERSEDED (uf1-5):** the union now exposes **13** — sw3-4 added the
  > trench-timer lines (`letGoLuke` among them restored later by sw7-2, U-007)
  > and sw7-8 added the five R8 wirings — so the deferred remainder is **10**,
  > not 19. Left as history; the current inventory, with the ROM routine and the
  > mechanism each remaining line waits on, is the `DEFERRED` block in
  > `src/core/events.ts`.
