...
> **AMENDED 2026-07-29 by sw8-8 — the inference below is wrong; the observation is not.**
> The *observation* stands and is still unexplained: the cabinet does put the Death Star
...
> **SUPERSEDED IN PART, 2026-07-29 by sw8-8 — do not re-follow the Fix line below.**
> The half of this story that shipped and STANDS is the starfield: space reads as lateral
> drift, ported as `core/starfield.ts STAR_LATERAL_SPEED`, which is where `ST.UX` actually
> belongs. The half that was WRONG is the camera: `cameraView`'s space branch was driven off
> the same register, and that displaced the pilot's eye and gun from the cockpit his shield
> is scored at and incoming fire homes at — measurably unfair (sw8-8: 0 % of fireballs
> answerable at impact; 100 % from the cockpit). `ST.UX` is the star generator's input, never
> a viewer; see the amendment on the §1 evidence anchor for the full source case, and the
> tombstone in `src/core/gameRules.ts`. **The space camera is the cockpit at the world
> origin.** The acceptance line below ("the DS sits off-centre / off-screen") is a real
> cabinet observation that our port no longer reproduces; satisfying it is **sw8-17**, and it
> must move the STATION, not the camera.
