...
//
// Story sa1-2 (epic sa1) — RED phase (Leeloo / TEA). joust GAINS a pause.
//
...
// missile-command) already adopt it, each with its OWN card + colour. joust is
...
// our-source line refs — so the comment-line-refs durability guard stays green.
...
describe('sa1-2 — joust adopts @shared/pause + /esc-overlay (AC-1, AC-2)', () => {
  it('a src module imports the shared pause gate', () => {
    expect(
      ...
      'no src file imports @shared/pause — joust has not wired the pause gate',
    ).not.toHaveLength(0)
  })
