...
// prior rounds these are REAL code gaps in the shipped sim.ts / main.ts, confirmed
// at source level. These rails fail against the CURRENT code (HEAD 9003193); Dev
// then fixes them. Pre-existing 1007 stay green (these rails use loadSimRender for
...
//      (sim.ts) so collisionPass `continue`s on every pair — no joust can EVER
//      resolve through stepSim (deleting the whole pass reds nothing today).
...
//   3. Wave 2 never follows: stepSim returns `wave` unchanged.
//
...
function playerProcess(id: number, posX: number, mount: 'ostrich' | 'stork', facing: -1 | 1): SimProcess {
  return {
    id,
    cls: 'primary',
    nap: 1,
...
// ═════════════════════════════════════════════════════════════════════════════
// RAIL 1 — A JOUST RESOLVES THROUGH stepSim (the collision pass is LIVE).
...
// ═════════════════════════════════════════════════════════════════════════════
describe('Round 2 Rail 1 — a joust resolves through stepSim', () => {
  it('an overlapping player over an enemy is killed by the loop: loser gone, egg spawned, kill score emitted', async () => {
    ...
    let staged: SimState = { ...demo, sim: { ...demo.sim, processes: [player, enemy] }, events: [] }

    let loserGone = false
    let eggSpawned = false
    let killScored = false
    for (let i = 0; i < 4 && !(loserGone && eggSpawned && killScored); i++) {
      staged = dmod.stepSim(staged)
...
// ═════════════════════════════════════════════════════════════════════════════
// RAIL 3 — createWaveSim OUTPUT: enemy type, player facing (drop-mutant guards),
//   and the player MOUNT (the round-2 red).
// ═════════════════════════════════════════════════════════════════════════════
describe('Round 2 Rail 3 — createWaveSim output', () => {
  it('all three wave-1 enemies carry enemyType bounder (kills the drop mutant)', async () => {
    ...
    const demo = seatWaveInstantly(dmod.createWaveSim(SEED))
...
//   Current gap: stepSim returns `wave` unchanged, so the demo is stuck on wave 1.
// ═════════════════════════════════════════════════════════════════════════════
describe('Round 2 Rail 4 — the wave advances when it is cleared', () => {
   ...
   */
  const cleared = (demo: SimState): SimState => strippedToPlayers(demo)
  ...
  it('does NOT advance while enemies are still alive (do not skip a live wave)', async () => {
    ...
    for (let i = 0; i < 10; i++) d = dmod.stepSim(d)
