...
// derivation that is only ever applied to the real records could be a
...
/** Only the `rom` entries, which is all of them today. */
  ...
  it('audio-manifest.ts exports `framesFor`', async () => {
    ...
    expect(
      ...
      'AC2 needs the derivation to be a callable seam — a total that is only ever ' +
        'applied to the real records cannot be told from a lookup table',
    ).toBe('function')
  })
...
describe('jt5-6 AC3 — player 2 sounds SNPCR2, not player 1’s table', () => {
  ...
  it('the sim really re-enters BOTH knights — the fixtures below are measured, not guessed', () => {
    ...
    //
    // jt11-11 RE-BASELINE (the enemy ground step signs velXIndex by PFACE): the
    // 0xface pair moves 1210 -> 605 (knight 1) and 2055 -> 958 (knight 2).
    // Buzzards now launch the way they were walking, so both knights' timelines
    // move from the first grounded enemy takeoff on. Re-swept by this test's own
    // precondition (which knight ids appear in the process list that were not
    // there the frame before), never by nudging:
    //   0xface re-entries: 605->1, 958->2, 2104->1, 3090->2
    // Same "earliest that re-enters knight N" rule as every prior move, and
  ...
  it('every cue in the manifest is reachable from some event — none is dead weight in the bake', async () => {
    ...
    // failed RED. Every cue and kind today is
...
describe('jt5-6 AC4 — the new cue is baked, and it is a genuinely different sound', () => {
  it('the new cue has a synth spec — proven by the bake completing, not by grepping for one', async () => {
    ...
    //
    // GREEN ON ARRIVAL, and say so: every cue today has a spec, so
