...
describe('joust key rebinding', () => {
  ...
  it('an override resolves onto the new code and drops the default', () => {
    const map = resolveBindings(CONTROL_MANIFEST, { p1Flap: ['KeyJ'] })
    expect(map.p1Flap).toEqual(['KeyJ'])
    expect(map.p1Flap).not.toContain('Space')
    expect(map.p2Flap).toEqual(['ShiftLeft']) // untouched actions keep defaults
  })
})
