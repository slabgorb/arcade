...
//
// jt3-2 added `SimState.arena` and an `applyWaveDestruction` call-site in stepSim,
...
//       `demo.arena.bridgeBurned` to true. A Dev who deletes the stepSim
 ...
 * keep the players and the SAME arena, and step once — stepSim advances the wave
...
function forceAdvance(step: (d: SimState) => SimState, demo: SimState): SimState {
  const stripped: SimState = strippedToPlayers(demo)
...
describe('jt3-3 — the demo arena seam is READ by the troll gate (the carried obligation)', () => {
  ...
  it('THE CALL-SITE — advancing the demo across wave 3 flips demo.arena.bridgeBurned', async () => {
    // If the stepSim applyWaveDestruction call-site is deleted, demo.arena stays
  ...
  it('THE GATE — trollSpawnable follows the LIVE demo arena: false pre-delay, true from wave 4', async () => {
    ...
    let d = demo.createWaveSim(SEED)
    d = forceAdvance(demo.stepSim, d) // wave 2
    d = forceAdvance(demo.stepSim, d) // wave 3 — bridge burned, but TTROLL+1 delay
    expect(d.wave).toBe(3)
    expect(troll.trollSpawnable(d.arena, d.wave), 'wave 3: burned but pre-delay').toBe(false)

    d = forceAdvance(demo.stepSim, d) // wave 4 — the troll wave
  ...
  it('a STALE arena defeats the gate — proving the call-site is what makes wave 4 spawnable', async () => {
    ...
    const demo = await loadSim()
    const troll = await loadTroll()
    const fresh = demo.createWaveSim(SEED) // arena bridge intact
