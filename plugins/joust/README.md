...
npx vitest run --project joust      # 124 files (derived + guarded); ~2763 tests, indicative
...
node plugins/joust/tools/audit/check-citations.mjs   # → "checked 961 claim(s)"
```
...
- **The citation gate cannot pass over an empty CLAIM SET.** With
  ...
  **That guard does not extend to a missing SOURCE TREE**, and the distinction
  ...
  `checked 961 claim(s) / all claims verified`, exit 0 —
