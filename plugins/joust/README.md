...
npx vitest run --project joust      # 113 files (derived + guarded); ~2658 tests, indicative
...
node plugins/joust/tools/audit/check-citations.mjs   # → "checked 959 claim(s)"
```
...
- **The citation gate cannot pass over an empty CLAIM SET.** With
  ...
  **That guard does not extend to a missing SOURCE TREE**, and the distinction
  ...
  `checked 959 claim(s) / all claims verified`, exit 0 —
