...
npx vitest run --project joust      # 129 files (derived + guarded); ~2757 tests, indicative
...
node plugins/joust/tools/audit/check-citations.mjs   # → "checked 965 claim(s)"
```
...
- **The citation gate cannot pass over an empty CLAIM SET.** With
  ...
  **That guard does not extend to a missing SOURCE TREE**, and the distinction
  ...
  `checked 965 claim(s) / all claims verified`, exit 0 —
