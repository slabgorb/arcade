...
beforeAll(async () => {
  ...
  //   • main.ts:122      — `window.addEventListener('keydown', unlockAudio)`
  //   • main.ts:124-127  — the canvas 'click' listener, which calls
  //                        `unlockAudio()` before `lock.request()`
  // (The third listener, the initials keydown at main.ts:133-135, is a separate
  ...
  it('the boot is SEEDED — this file observes a pinned world, not whatever the clock said', () => {
    ...
    expect(
      ...
      `main.ts did not boot the world ?seed=${SEED} asks for — it is still seeding attract ` +
        ...
        'override (the ?wave= shape, main.ts:41-50)',
    ).toBe(true)
  })
