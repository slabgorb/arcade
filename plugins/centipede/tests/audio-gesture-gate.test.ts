...
beforeAll(async () => {
  ...
  //   • main.ts:123      — `window.addEventListener('keydown', unlockAudio)`
  //   • main.ts:125-128  — the canvas 'click' listener, which calls
  //                        `unlockAudio()` before `lock.request()`
  // (The third listener, the initials keydown at main.ts:134-136, is a separate
...
describe('cp5-2 AC4 — the gesture gate is respected', () => {
  it('the boot is SEEDED — this file observes a pinned world, not whatever the clock said', () => {
    ...
    // main.ts:228 seeded attract straight from `Date.now()`, so "the attract
    ...
    expect(
      ...
      `main.ts did not boot the world ?seed=${SEED} asks for — it is still seeding attract ` +
        ...
        'override (the ?wave= shape, main.ts:42-51)',
    ).toBe(true)
  })
