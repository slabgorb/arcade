...
describe('src/core/ purity sweep (cp1-1 AC-2 — the boundary guard)', () => {
  // pm1-2 DEVIATION FROM THE VERBATIM CENTIPEDE COPY (the one change): pac-man's
  // src/core/ is EMPTY today — pm1-2 installs the harness BEFORE any gameplay
  // code, so this sweep must PASS VACUOUSLY now and ARM ITSELF the moment core
  // modules land (task-2 Step 2). The scanner's own teeth are proven forever by
  // the fixture self-tests above; centipede's copy asserts a non-empty core
  // because centipede's core already existed. Here that assertion is replaced by
  // a skip-when-empty so the gate is green pre-implementation, then bites as soon
  // as the first core/*.ts is committed.
  it.skipIf(coreFiles.length === 0)('scans a non-empty src/core/ (the sweep must have teeth once core lands)', () => {
    expect(
      coreFiles.length,
      'src/core/ must hold at least one .ts module for the sweep to bite',
    ).toBeGreaterThan(0)
  })
