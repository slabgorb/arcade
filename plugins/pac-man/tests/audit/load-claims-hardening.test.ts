...
// guards checkClaims dispatches on (pac-man accepts a TEXT {file,line,verbatim}, a BINARY {file,offset,nibbles} —
// the WSG PROM shape used by the real sound.json — or a BYTE {file,offset,bytes} source).
//
...
// A nibble-shaped source ({file, offset, nibbles}) — pac-man's isValidClaimSource ORs
// isBinaryCitation, so this must load exactly like the text shape. Guards R2: a
// regression dropping the nibble branch would reject real nibble claims (the WSG
// PROM sound.json) and this suite would otherwise stay green.
const GOOD_NIBBLE_CLAIM = { id: 'b', claim: 'a nibble fact', source: { file: '82s126.1m', offset: 0, nibbles: [0, 15] } }
...
describe('loadClaims hardening (df1-6) — pac-man', () => {
  ...
  it('still loads a NIBBLE-shaped source claim (offset/nibbles), not only text', () => {
    const dir = withClaimsDir({ 'nibbles.json': JSON.stringify([GOOD_NIBBLE_CLAIM]) })
    try {
      const out = loadClaims(dir)
      expect(out).toHaveLength(1)
