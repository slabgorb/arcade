...
// HUD readout routes through @shared/font, not the browser system font.
//
// THE TRAP (SM handoff / context §Test-Seam): routing the HUD through the shared
// subpath RELOCATES the observed text INTO @shared/font. A spy on
...
import { GLYPH_CHARS } from '@shared/font'
...
// charGlyph. (A charGlyph tap here would be inert anyway: @shared/font's
...
vi.mock('@shared/font', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@shared/font')>()
  return {
...
describe('rb4-19 AC-1 — the live HUD readout routes through @shared/font', () => {
  it('routes the SCORE line through the shared glyph font (not the canvas font)', () => {
    expect(
      routedContent().includes('SCORE'),
      'the SCORE readout never reached @shared/font — the HUD is not routed through the shared glyphs',
    ).toBe(true)
  })
  ...
  it('routes the PLANE readout through the shared glyph font while a wave is up', () => {
    expect(
      ...
      'the PLANE readout never reached @shared/font — AC-4 (relocated from rb4-9) unproven at the new seam',
    ).toBe(true)
  })
...
describe('rb4-19 AC-3 — every routed HUD string lies within the font GLYPH_CHARS', () => {
  it('routed something, and every routed character is a glyph the font can draw', () => {
    expect(
      ...
      'nothing was routed to @shared/font — cannot prove glyph coverage (HUD still on the canvas font)',
    ).toBeGreaterThan(0)
