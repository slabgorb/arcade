...
// of the shared ROM vector glyph font at @shared/font (v0.15.0, already
...
describe('rb4-19 AC-1 — main.ts stops drawing HUD text with the browser system font', () => {
  it('sets no `monospace` canvas font in main.ts', () => {
    ...
    expect(
      ...
      'main.ts still sets a `monospace` ctx.font for the HUD — it must route through @shared/font',
    ).toBe(false)
  })
  ...
  it('makes no `ctx.fillText`/`ctx.strokeText` call in main.ts (the HUD was its only text)', () => {
    ...
    expect(
      ...
      `main.ts still draws text through the canvas font (${hits.length} fillText/strokeText call(s)) — ` +
        'the HUD readout must be stroked as @shared/font vector glyphs instead',
    ).toHaveLength(0)
  })
})
...
describe('rb4-19 AC-1 / AC-2 — the shared-font wiring is adopted, the overlay is left intact', () => {
  it('a game src module imports @shared/font (the HUD glyph subpath is wired)', () => {
    expect(
      importsSubpath('@shared/font'),
      'no src file imports @shared/font — the HUD is not routed through the shared glyph font',
    ).not.toHaveLength(0)
  })
