 ...
 *
 * MONOREPO MIGRATION — `--full-tree` is load-bearing, and its absence was SILENT. git
 * resolves an `ls-tree` pathspec against the cwd's PREFIX INSIDE THE REPO. While red-baron
 * was its own repo that prefix was empty, so `docs/audit/findings/` meant the repo root's.
 * As `plugins/red-baron` inside the arcade monorepo the prefix is `plugins/red-baron/`, so
 * the same pathspec asked the audit commit (a red-baron-history commit, kept reachable by
 * the `audit/red-baron` tag) for `plugins/red-baron/docs/audit/findings/` — a path that
 * never existed there. git exits 0 and prints NOTHING for an unmatched tree pathspec, so
 * this returned an empty list rather than erroring: the anti-tamper lock would have had no
 * audited findings left to compare against. `--full-tree` pins the pathspec to the ref's
 * own root regardless of cwd, which is what this always meant. (`git show <rev>:<path>`
 * below is already root-relative unless the path starts with `./`, so it needed no change.)
 */
function rawFindingsAtAuditCommit(): readonly RawFinding[] {
  return gitOrThrow(['ls-tree', '--full-tree', '--name-only', AUDIT_COMMIT, `${FINDINGS_PATH}/`])
    .split('\n')
    .map((s) => s.trim())
    .filter((f) => f.endsWith('.json'))
    .flatMap((file) => {
