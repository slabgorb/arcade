...
// @shared/font (vector glyphs, stroked — NOT the canvas font), so the fillText
...
const ctxStub: Record<string, unknown> = {
  ...
  save: () => {}, restore: () => {},
  // rb4-19: the HUD readout now strokes @shared/font glyphs — tolerate a
...
// through @shared/font (stroked glyphs), not ctx.fillText, so it is observed
