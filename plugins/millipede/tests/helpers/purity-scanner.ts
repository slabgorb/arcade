...
//
// Story ml1-1 (GREEN) — the core/shell boundary scanner for Millipede, ported
// verbatim from plugins/centipede/tests/helpers/purity-scanner.ts (itself td1-2's
// port of joust jt1-7/jt1-11 onto the TypeScript compiler API). One implementation
// per game — the fleet carries a copy each, scanning that game's own src/core/.
// Shared by tests/purity.test.ts (which sweeps src/core/, dormant until ml3 lands
// the sim) and, when a companion suite is added, its behaviour pins.
//
...
// ─── BAN SET (preserved exactly from centipede's scanner) ────────────────────
