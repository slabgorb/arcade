...
// playfield-GRAPHICS bank ($C0-$FF: mushrooms, DDT, rocks, bonus numbers).
//
// CORRECTED 2026-08-15 (millipede-render-mapping-correct-mame): MAME's
// milliped_get_tile_info (centiped_v.cpp:35-43) is tile = (code&0x3f) + 0x40 +
// bank*0x80, bank = ((code>>6)&1). So bit-6 SET adds 0x80 on top of the 0x40
// base → $C0-$FF, NOT the $00-$3F the ml7-6 draft assumed (those are the
// MOTION-OBJECT sprite tiles; mapping mushrooms there drew sprite fragments as
// mushrooms). Real mushrooms are $FC-$FF.
//
...
// instead of the mushroom graphics ($FC-$FF). The ml2-4 census drew the raw
// TILES so the mushroom at $FF looked right there, and every charTile test used
...
describe('ml7-6 — charTile bit-6 bank select (the mushroom decode fix)', () => {
  ...
  it('the field GRAPHICS half (bit 6 set) maps into the $C0-$FF bank', () => {
    // Every playfield-graphic stamp is bit-6 SET and must land in the HIGH
    // graphics bank ($C0-$FF) — the 0x40 base plus the bank*0x80 the set bit-6
    // adds — not the char bank the ml7-3 rule left it in, and not the $00-$3F
    // motion-object tiles the ml7-6 draft wrongly aimed at.
    for (const code of [DDT_STAMP, DDT_STAMP + 1, ROCK, POISON, NORMAL, FULL_MUSHROOM]) {
      ...
      expect(charTile(code)).toBe(0xc0 | (code & 0x3f)) // $6E->$EE .. $7F->$FF
    }
  })

  it('the four NORMAL mushroom stages ($7C-$7F) decode to the mushroom tiles ($FC-$FF), all non-blank', () => {
    // The bug: FULL_MUSHROOM ($7F) rendered tile $7F, an 11-pixel red fragment.
    // The fix: it renders tile $FF, a real mushroom. Pin the whole damage band.
    for (let stamp = NORMAL; stamp <= FULL_MUSHROOM; stamp++) {
      const tile = charTile(stamp)
      expect(tile).toBe(0xc0 | (stamp & 0x3f)) // $7C->$FC .. $7F->$FF
