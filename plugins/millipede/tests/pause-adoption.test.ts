...
//
// Story sa1-2 (epic sa1) — RED phase (Leeloo / TEA). millipede GAINS a pause.
//
...
// missile-command) already adopt it, each with its OWN card + colour. millipede
...
describe('sa1-2 — millipede adopts @shared/pause + /esc-overlay (AC-1, AC-2)', () => {
  ...
  it('a src module imports the shared esc-overlay', () => {
    expect(
      ...
      'no src file imports @shared/esc-overlay — millipede draws no pause overlay',
    ).not.toHaveLength(0)
  })
