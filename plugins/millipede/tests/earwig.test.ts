...
describe('earwig — movement', () => {
  ...
  it('the picture flaps only every fourth frame (EW-28/29)', async () => {
    ...
    expect(still.pic, 'residue 1 (frame 5) does not flap').toBe(0x1c)
    m.moveEarwig(still, env({ frame: 2 }))
    expect(still.pic, 'residue 2 (frame 2, EVEN) does not flap — AND I,03, not AND I,01').toBe(0x1c)
    m.moveEarwig(still, env({ frame: 7 }))
    expect(still.pic, 'residue 3 (frame 7) does not flap').toBe(0x1c)
    m.moveEarwig(still, env({ frame: 6 }))
    expect(still.pic, 'residue 2 again (frame 6) — the even non-multiple-of-4 frames are the & 1 discriminators').toBe(0x1c)
  })
})
