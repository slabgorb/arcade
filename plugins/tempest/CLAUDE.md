...
├── docs/                # design docs, specs and the fidelity audit
├── index.html           # game entry
├── models.html          # second entry: the model contact-sheet dev tool
...
Tempest is one app inside the `arcade` monorepo. There are **no per-game scripts**
— `plugins/tempest/package.json` is a `{name, version, private}` stub. Run
...
`npx vitest run --project tempest citations` (from the repo root) re-opens both and compares.
A citation that cannot be re-opened is not evidence, so **the gate is not optional and must
stay green**.
...
NOT the working tree (story tp1-22). The record is `git show 4232ed4:<file>`. That commit
came across into the monorepo with the rest of tempest's history and is reachable here via
the `audit/tempest` tag; its paths are the OLD repo-root paths (`src/core/sim.ts`, not
`plugins/tempest/src/core/sim.ts`), which is correct — `git show <rev>:<path>` resolves
against that commit's own tree, so the gate needed no re-anchoring for the migration. CI's
...
Tempest has no repo of its own any more — it is a directory in the `arcade`
monorepo and follows the root `CLAUDE.md`'s workflow (trunk-based on `main`).

- **Release tags are `tempest-vX.Y.Z`.** A bare `vX.Y.Z` is invalid: this monorepo
  holds every app's tags side by side.
- There is no per-game `.github/workflows/deploy.yml` and no per-game `develop`.
