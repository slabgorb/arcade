...
describe('checkFindings', () => {
  ...
  // constant drifted: SC-001 and SC-009 cited `node_modules/@arcade/shared/dist/
  // highscore.js`, the BUILT output of a version-pinned git dependency. That dist/ was
  // gitignored in arcade-shared and regenerated on install, so its line numbers moved
  // under the audit every time the library was re-pinned or rebuilt — line 46 held
  // `MAX_HIGH_SCORES = 10` when the audit ran and held a comment by then.
  //
  ...
  // `node_modules/@arcade/shared/dist/highscore.js` and READ that file off disk so its
  // verbatim was byte-exact. tempest no longer has a node_modules of its own (the root
  // owns the toolchain) and the shared library is now in-repo at src/shared, so that
  // file does not exist to read. The fixture below cites a node_modules path WITHOUT
  // reading anything, and the two assertions replace the old byte-exactness trick with
  // a stronger, file-free guarantee of the same non-vacuity: the error must be the PATH
  // rule's own message, and must NOT be a verbatim/existence complaint. A checker that
  // merely failed to open the file would emit "does not match verbatim at <sha>" and is
  // rejected here; only a rule that refuses the PATH passes.
  it('rejects an `ours` citation into node_modules — a rebuilt artifact is not our source', () => {
    ...
    const errors = checkFindings(
      [{
        ...
        source: { file: 'ALWELG.MAC', line: 1, verbatim: 'anything' },
        ours: { file: nmFile, line: 46, verbatim: 'export const MAX_HIGH_SCORES = 10;' },
        claim: 'c', reasoning: 'r', recommendation: 'fix', size: 's',
      }],
      { repoRoot, sourceDir: null },
    )
    ...
    expect(joined, 'the path rule must fire ahead of the frozen re-open, not after it')
      .not.toMatch(/does not match verbatim|cannot be frozen/)
  })
