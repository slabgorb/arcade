...
// promoted VERBATIM into @shared/font (its geometry contract now lives in
// arcade-shared/tests/font.test.ts). Here we drive the tempest-side changes:
//
//   AC-3  font.ts re-exports @shared/font (not ./vecfont); vecfont.ts is
...
// migration this resolves to ./vecfont; after it, to @shared/font. Either way
...
// ===========================================================================
// AC-3 — tempest consumes @shared/font; the local vecfont.ts is gone
// ===========================================================================
describe('SH2-2 AC-3 — font re-points to @shared/font', () => {
  it('font.ts re-exports the SHARED module, not the local ./vecfont', () => {
    expect(fontSrc, 'font.ts should re-export @shared/font').toMatch(/@shared\/font/)
    expect(fontSrc, 'font.ts should no longer re-export ./vecfont').not.toMatch(
      /from\s+['"]\.\/vecfont['"]/,
    )
  })

  it('src/shell/vecfont.ts is deleted (its contents moved to @shared/font)', () => {
  ...
  it('render.ts imports the font from the shared module / barrel, not ./vecfont', () => {
    ...
    expect(renderSrc, 'render.ts should import from ./font or @shared/font').toMatch(
      /from\s+['"](\.\/font|@shared\/font)['"]/,
    )
  })
