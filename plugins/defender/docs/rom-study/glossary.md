...
| `HOFIN` — HALL OF FAME INITIALS DISPLAY (`*HALL OF FAME INITIALS DISPLAY`): (re)draws the 3-char in-progress initials BUFFER during entry — the df7-4 `ENTER INITIALS` line maps here; the persisted table above it is df7-4's own un-ported layout | hall-of-fame initials-buffer display | `defender/AMODE1.SRC:244` |
...
| `HOFUD` — HALL OF FAME UP DOWN STICK HANDLER (`*HALL OF FAME UP DOWN STICK HANDLER`): the up/down stick cycles the selected initial's LETTER (A–Z), NOT the cursor slot (a separate FIRE-switch handler advances that); mapped in df7-4 onto the `@shared` name-entry letter-append keyboard input (the fleet keyboard ruling) | initials letter up/down stick | `defender/AMODE1.SRC:325` |
