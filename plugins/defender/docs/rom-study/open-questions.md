...
CB1 (williams_m.cpp), and the handler's control-register values fit the CB1
model. One of the two namings is a slip, most likely the source comment's.
**Disposition:** df1-4 (the MAME board-facts story) resolves it and records the
comment as a likely author slip; the port implements whichever model df1-4 pins.
...
setup against MAME's PIA model.
...
watchdog/screen-flip seam is built. **Disposition:** df1-4 resolves it from
MAME's decode tables.
