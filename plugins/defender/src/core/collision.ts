...
// enemy exists. A pure, clock-free port of the ROM's OWN COLIDE routine
...
// Decision A (RULED, ROM-always-wins, df4 design spec §2): a tidier structure would
// silently change WHICH overlaps count (routing != geometry, the df3 lesson). The
// src/core purity sweep (tests/purity.test.ts) scans this file; it reads no clock,
...
//   First hit wins (:2923-2925)  the OLINK walk returns on the FIRST overlapping object
//        in list order (RTS NE = crash); an exhausted list is EQ = no crash (null here).
...
//        the pixel offset (`LEAY $100,Y` per column) and `STY CENTMP`. With no bitmap
//        pixels (df4-1 tests against synthetic lists) the box-level address is that UL
//        coord-as-address: (x<<8)|y. The pixel-precise centre refinement needs real
//        bitmap data a later story carries.
