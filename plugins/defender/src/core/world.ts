...
//   BGL  (PHR6.SRC:215) "TERRAIN LEFT POINTER" — the CAMERA: the world-X of the
//        screen's left edge. The world is a 16-BIT HORIZONTAL CYLINDER: BGL and every
//        world-X wrap at $10000 with no clamp (wrap16).
//   BGLX (PHR6.SRC:216) "OLD TERRAIN LEFT" — last frame's camera, for the object/star
//        scroll delta. slide() saves it (DEFA7.SRC:2419-2420).
//   PLABX(PHR6.SRC:335, DEFA7.SRC:2432-2440) — absolute world X is DERIVED not stored:
//        worldX(entity) = onscreen + BGL, a pure helper (wrap16'd).
//   PLAY1(DEFA7.SRC:2373-2431) — the SHIP LEADS THE SCROLL:
...
//   YMIN=42 / YMAX=240 are DECIMAL (PHR6.SRC:20-21).

/** PLADIR sign: the ship faces right (BPL / positive) or left (BMI / negative). */
export type Facing = 'right' | 'left'

/** YMIN — minimum Y coordinate (PHR6.SRC:21, decimal). */
export const YMIN = 42
/** YMAX — maximum Y coordinate (PHR6.SRC:20, decimal). */
export const YMAX = 240

/** The base target column per facing: $20 right (DEFA7.SRC:2385), $70 left (:2389). */
const BASE_RIGHT = 0x20
const BASE_LEFT = 0x70
/** BGDELT slide step and the in-range window half-width (DEFA7.SRC:2400-2412). */
const SLIDE_STEP = 0x40
const SLIDE_WINDOW = 0x100
/** Player velocity clamp bound (DEFA7.SRC:2422-2427). */
...
/**
 * PLAY1 velocity→column mapping (DEFA7.SRC:2373-2396): the ship's target screen
...
/** clampPlayerY — the player's Y is CLAMPED to [YMIN+1, 238] (DEFA7.SRC:2450,2461). */
...
/** wrapObjectY — an object's Y WRAPS one step on [YMIN, YMAX] (DEFA7.SRC:2490-2496). */
...
export interface SlideResult {
  ...
  readonly bgl: number
  /** BGLX — the pre-tick camera (DEFA7.SRC:2419-2420). */
  ...
  /** PLAXV — the velocity after the ±$100 clamp (DEFA7.SRC:2422-2428). */
  readonly plaxv: number
}

/**
 * PLAY1 (DEFA7.SRC:2397-2431): slide BGL toward the ship's target column, then
