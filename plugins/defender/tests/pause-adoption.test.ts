...
//
// Story sa1-2 (epic sa1) — RED phase (Leeloo / TEA). defender GAINS a pause.
//
...
// missile-command) already adopt it, each with its OWN card + colour. defender
...
// (drawControlsOverlay, not drawEscOverlay). defender drops its drawEscOverlay
// import entirely; @shared/pause's VERB (isPauseKey — stepUnlessPaused's thunk
// gate is exercised by the module-resolution assertion below, not by defender's
// own createLoop-based freeze gate) is unchanged and still gates the frozen
// frame, so AC-1's contract still holds.
//
...
describe('sa1-2 — defender adopts @shared/pause + @shared/controls-overlay (AC-1, AC-2)', () => {
  it('a src module imports the shared pause gate', () => {
    expect(
      importersOf(PAUSE_IMPORT),
      'no src file imports @shared/pause — defender has not wired the pause gate',
    ).not.toHaveLength(0)
  })
  ...
  it('a src module imports the shared controls overlay (sa1-5: owns pause chrome)', () => {
    expect(
      ...
      'no src file imports @shared/controls-overlay — defender draws no pause overlay',
    ).not.toHaveLength(0)
  })
