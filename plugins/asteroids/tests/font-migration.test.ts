...
// vendored "Vector Battle" TTF (ctx.font + ctx.fillText) onto @shared/font
...
describe('SH2-4 — render.ts draws text via @shared/font layoutText', () => {
  ...
  it('imports and calls layoutText', () => {
    ...
    expect(
      /import[^;]*\blayoutText\b[^;]*from\s*['"](?:\.\/font|@shared\/font)['"]/.test(src),
      'render.ts must import layoutText from ./font or @shared/font',
    ).toBe(true)
