...
// engine onto the shared @shared/audio `createAudioEngine` (SH2-16), keeping its
// per-cabinet NUMBERS (the SFX manifest, the R2 base) and its
...
//   2. resolution    — @shared/audio resolves and exports createAudioEngine.
...
//
// MONOREPO NOTE: a sixth assertion here pinned the shared library as a git-URL
...
// Driver 2 keeps the resolution half of that contract: it is now an ALIAS
// resolution rather than a dep-pin resolution, and the export it demands is the
// same one.
...
describe('SH2-17 — asteroids adopts @shared/audio (AC-1, AC-2)', () => {
  it('src/shell/audio.ts imports the shared audio engine (engine no longer hand-rolled)', () => {
    expect(
      AUDIO_IMPORT.test(audioSrc()),
      'src/shell/audio.ts does not import @shared/audio — the shared engine was not adopted',
    ).toBe(true)
  })

  it('@shared/audio resolves and exposes createAudioEngine', async () => {
    // Non-literal + @vite-ignore so an unresolvable subpath does not fail this file at
    // COLLECTION time (which would suppress every other driver below) — it must reject
    // at RUNTIME, as its own granular miss.
    const spec = '@shared/audio'
    const audio = await import(/* @vite-ignore */ spec)
    expect(
      typeof audio.createAudioEngine,
      'createAudioEngine must be exported by @shared/audio — check the @shared alias and src/shared/audio.ts',
    ).toBe('function')
  })
