...
// asteroids must stroke its glowing vectors through the shared @shared/glow
...
//   2. resolution — @shared/glow resolves with withGlow + glowPolyline. This was a
//                   dep-pin resolution against @arcade/shared; post-monorepo it is
//                   an ALIAS resolution against src/shared. Same exports demanded.
...
describe('SH2-8 — asteroids adopts @shared/glow (AC-2)', () => {
  it('a src module imports the shared glow primitive (strokes are no longer hand-rolled)', () => {
    ...
    expect(
      importers,
      'no src file imports @shared/glow yet — asteroids has not adopted the shared primitive',
    ).not.toHaveLength(0)
  })

  it('@shared/glow resolves with withGlow + glowPolyline', async () => {
