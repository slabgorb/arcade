...
//
// Story SH2-14 (epic SH2) — RED phase (Furiosa / TEA). asteroids GAINS a pause:
...
// shared font). This story wired asteroids onto them: an Escape keydown EDGE
// toggled pause, the frozen-frame gate held the sim, and drawEscOverlay drew
// over the held world — with asteroids supplying its OWN keybind card + colour
...
// (drawControlsOverlay, not drawEscOverlay). asteroids drops its drawEscOverlay
...
//                   DEP-PIN contract (asteroids pinned @arcade/shared ≥ v0.9.0);
...
describe('SH2-14 — asteroids adopts @shared/pause + /esc-overlay (AC-1, AC-2)', () => {
  it('a src module imports the shared pause gate', () => {
    expect(
      importersOf(PAUSE_IMPORT),
      'no src file imports @shared/pause — asteroids has not wired the pause gate',
    ).not.toHaveLength(0)
  })
  ...
  it('a src module imports the shared controls overlay (sa1-5: owns pause chrome)', () => {
    expect(
      ...
      'no src file imports @shared/controls-overlay — asteroids draws no pause overlay',
    ).not.toHaveLength(0)
  })
