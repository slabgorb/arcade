...
// in the shared reducer (@shared/name-entry) so all four scoring games
...
//    '@shared/name-entry' (AC-3), and the shell forwards Backspace
...
// ---- The shared VERB (AC-3): mechanism lives in the shared library ------------
...
describe('the entry mechanism is the SHARED reducer, not a local fork (AC-3)', () => {
  it('some core module imports @shared/name-entry', () => {
    expect(coreSources()).toContain('@shared/name-entry')
  })
})
