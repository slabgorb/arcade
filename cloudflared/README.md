...
| `/battlezone/*` | `http://localhost:5276` | Battlezone dev server (Vite base `/battlezone/`) |
...
(`/tempest/`, `/star-wars/`, `/asteroids/`, `/battlezone/`) must come before the
lobby catch-all (see [`config.yml`](./config.yml)).
