...
const srcPath = fileURLToPath(new URL('../name-entry.ts', import.meta.url))
...
// The "packaging" describe that lived here asserted package.json's exports map
// pointed './name-entry' at dist/name-entry.js — a package-boundary contract that
// no longer exists post-migration (no dist/, no package.json in src/shared; see
// the purity.test.ts rewrite for the same reasoning). @shared/name-entry is now
// resolved directly by the root tsconfig/vitest path alias, so there is no
// packaging contract left to police here.
